...
| `prune` (v3.10.0-rc.14) | `--vault <path>` `[--yes]` | GC cached index artifacts for OTHER vaults, keeping only the named vault's (`clear-cache`/`clear-index` only target the current vault). **Dry-run by default** — pass `--yes` to actually delete. Only ever removes enquire's own `<hash>.{fts5.db,embed.db,hnsw.bin,hnsw.meta.json}` files. |
