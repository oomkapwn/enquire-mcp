...
| `--no-hnsw-persist`    | persist on | v2.16.0 — disable HNSW persistence sidecar. Default behavior: save `.hnsw.bin` + `.meta.json` next to `.embed.db`; reload it on the next serve when the embed-db signature matches. **Since v3.12.0-rc.26:** an uncertain watcher HNSW sync also suppresses saving automatically, leaving restart signature validation/rebuild as the recovery path. |
...
**v2.16.0 — HNSW persistence:** when `--use-hnsw` is passed, the index is persisted to a sidecar `.hnsw.bin` + `.hnsw.meta.json` next to `.embed.db` after the first build. Subsequent serve starts load the sidecar when the embed-db signature matches; rebuild happens automatically on signature mismatch. Pass `--no-hnsw-persist` to disable. Since v3.12.0-rc.26, an uncertain watcher embedding-to-HNSW sync quarantines the live HNSW route, falls back to EmbedDb and permanently suppresses persistence for that server generation, so restart validation/rebuild is used instead of saving or querying a potentially partial graph.
