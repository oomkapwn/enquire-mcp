...
- **Explicit termination drains real calls before teardown.** `DELETE /mcp` marks the session closing and removes it from the registry first, then waits up to `DELETE_DRAIN_MS` (**5 seconds**) for pre-existing POST / initialize calls (`inFlightCalls`) to finish before terminating the SDK transport. Long-lived GET SSE streams are deliberately excluded from that drain because teardown is what closes them. A mutation that exceeds the bound remains the documented residual; the durable abort-signal + rollback-safe batch-write fix is tracked separately. DELETE is idempotent at the HTTP boundary: an unknown or already-terminated `Mcp-Session-Id` returns **204**.
...
- **Graceful shutdown.** SIGINT / SIGTERM / `beforeExit` close the initialize gate, remove the current session snapshot from the registry, drain pending initializes + `inFlightCalls` under the same bounded wait, then close every transport. Long-lived SSE streams are terminated rather than awaited.
