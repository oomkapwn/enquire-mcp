...
| `--no-hnsw-persist`    | persist on | v2.16.0 — disable HNSW persistence sidecar. Default behavior: save `.hnsw.bin` + `.meta.json` next to `.embed.db`; reload it on the next serve when the embed-db signature matches. v3.12.0-rc.25 also suppresses saving automatically after an uncertain watcher HNSW sync, leaving restart signature validation/rebuild as the recovery path. |
