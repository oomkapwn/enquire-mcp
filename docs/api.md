...
| `obsidian_vault_shape` | read | always | Frontmatter keys that exist, with counts, value shapes, and examples. |
...
| `clear-feedback` | `--vault <path>` `[--feedback-file <path>]` | Delete the closed-loop feedback sidecar (`<hash>.feedback.json` + any generated sibling) for the given vault. `clear-cache` deliberately leaves it alone — the marks are USER-recorded state, not derived data — and `prune` only reaches OTHER vaults' stems, so this is the command that erases the ACTIVE vault's feedback. Takes the exclusive family lease, so a concurrent publisher cannot commit a generation mid-erasure; a `serve` running with `--feedback-weight > 0` republishes on the next recorded mark, so stop it first when the intent is permanent erasure. Returns 0 when there is nothing to remove. |
...
Report which frontmatter keys exist in the vault, how often each occurs, what value shapes it holds, and a few example values.

| Argument    | Type      | Notes                                             |
|-------------|-----------|---------------------------------------------------|
| `folder`    | `string?` | Restrict to a subfolder.                          |
| `min_count` | `number?` | Drop keys used in fewer notes than this (default 1). |
| `limit`     | `number?` | Max results (default 200, ≤ 2000).                |

**Returns:** `Array<{ key, count, types, examples }>`, sorted by `count` desc then `key`. `types` is the sorted set of value shapes observed — `string`, `number`, `boolean`, `list`, `map`, `null`. `examples` holds up to three short scalar values, whitespace-collapsed and truncated at 80 characters.

> **Why this exists.** Every other frontmatter tool — `obsidian_frontmatter_get`, `obsidian_frontmatter_search`, the `filter_frontmatter` argument of `obsidian_search` — requires the key as an argument, which only helps a caller who already knows the vault. This answers the question that comes first: what is there to ask about.

> **Enumeration, not ranking.** The result is exhaustive within its scan bounds rather than a best guess, and the walk refuses instead of returning a prefix. An inventory that quietly omits keys is worse than none, because a caller cannot distinguish "this key does not exist" from "the scan stopped early".

> **Key folding.** Keys are reported NFC-lowercased. Lookup folds them the same way, so `Status` and `status` are one key to every tool that reads them and are counted as one here — either spelling works as an argument.

> Honors `--exclude-glob` / `--read-paths`: a key occurring only in filtered notes is not reported, exactly as those notes are not searchable.
