...
**v2.16.0 — HNSW persistence:** when `--use-hnsw` is passed, the index is written as an immutable `.hnsw.<nonce>.bin` generation next to `.embed.db`; `.hnsw.meta.json` is published last as the stable basename + SHA-256 pointer. Persisted pointer bytes and every pointer read enforce a 256 MiB cap, while writer peak memory remains proportional to the already in-memory metadata during serialization. An oversized pointer is not committed; precommit orphan cleanup is best-effort, and strict generated residue remains covered by explicit clear/prune. Subsequent serve starts load only a digest-matching generation whose embed-db signature matches; malformed, legacy-v1, missing, mixed, or stale pairs rebuild automatically. An uncertain watcher embedding-to-HNSW sync quarantines the live HNSW route, falls back to EmbedDb and permanently suppresses persistence for that server generation, so restart validation/rebuild is used instead of saving or querying a potentially partial graph.
