# External Audit Request — enquire-mcp v3.11.0 STABLE (FULL, from scratch, obligation-gated, comparison-ready — post-release @latest audit)

**Status:** OPEN — **post-release audit of v3.11.0 STABLE (the shipped `@latest`)** + the standing ≥2-auditor verification for the next line.
**Date issued:** 2026-06-27
**Supersedes:** `AUDIT-REQUEST-FULL-FROM-SCRATCH-v3.11.0-rc.24-2026-06-27.md`. **v3.11.0 is now the SHIPPED `@latest` (promoted 2026-06-27).** This is therefore a POST-RELEASE from-scratch audit of the live stable — catch anything that reached `@latest`, and serve as the standing ≥2-auditor verification for the next cycle.
...
git checkout v3.11.0 && git rev-parse HEAD       # expect 71f59bd… — record the FULL SHA in your report
npm view @oomkapwn/enquire-mcp@latest version        # must read 3.11.0 (the shipped stable)
```

> The authoritative target is the commit **tagged `v3.11.0`** (short SHA `71f59bd`) — the current `@latest`. Grade THAT commit; cite its SHA in every finding. `@latest` is **3.11.0** (this audit grades the shipped stable, not a candidate); the ≥2-independent-auditor discipline (v3.6.1) is standing for the next line.
...
**Authoritative docs to read first:** `README.md`, **`CLAUDE.md`** (the maintainer's North-Star + the running anti-pattern ledger and overclaim corrections #1–#22 — your map of where the apparatus has historically failed), `SECURITY.md`, `STABILITY.md` (semver contract), `docs/api.md`, `CHANGELOG.md` (esp. the rc.1→rc.25 + the `[3.11.0]` STABLE v3.11.0 entries).
...
- **Fixed in rc.23/rc.24/rc.25 (the 3-auditor rc.21 response — do NOT re-report; but DO re-probe the SIBLINGS):** the **LINE-TERMINATOR class** — `split("\n")` (splitting, rc.23) AND `match(/\n/g)` (line counting, rc.25) were LF-only, so a note with bare CR or U+2028/U+2029 as the line *separator* collapsed to one line — merging headings/open-questions (read-path LOW) AND defeating per-line code-fence detection in `rewriteOutsideCodeFences`/`replaceStringOutsideCodeFences` (write-path **data corruption** MED, Goose alone) AND drifting `chatThreadAppend`/FTS breadcrumb line numbers (counting form). Fixed at the root with `splitLines`/`countLineBreaks` (`wildcard-match.ts`, terminator set `\n`/`\r\n`/`\r`/U+2028/U+2029; NEL/VT/FF EXCLUDED as non-CommonMark/Obsidian breaks) routed through all note-content sites + a `tests/line-terminator.test.ts` inventory invariant that flags BOTH `.split("\n")` and `.match(/\n/g)`. Also closed: `filter_frontmatter` + `frontmatter_set` value caps (rc.24) + `feedback.ts:open` file-size guard (rc.24). **Re-probe: does `splitLines`/`countLineBreaks` reach EVERY line-oriented sink, and is the line-ending boundary correct (CR/CRLF/LF/LS/PS split; NEL/VT/FF do NOT)?**
- **Fixed in v3.11.1-rc.1 (this STABLE's own audit response — do NOT re-report; re-probe the SIBLINGS):** the **case-fold-asymmetry class** — `replace_in_notes` (`replaceStringOutsideCodeFences`) folded the needle whole-string (`search.toLowerCase()` → Greek word-final `Σ→ς`) while the line folded per code unit (`Σ→σ`), so a case-insensitive Greek-final-sigma replace silently matched nothing. Fixed with a shared `foldForMatch` (per code point, context-free) in `wildcard-match.ts`, routed through the needle + `replaceLineOnce`'s fold loop (now per code point, astral-correct, keeps the rc.18 İ map). **Re-probe: is EVERY case-insensitive matcher folding the needle and the haystack the SAME way (per code point — never a whole-string `.toLowerCase()` on one side)?** The read-path `foldedIndexOf`/`foldWithMap` snippet-centring sibling is a documented-accept cosmetic residual (scoring is unaffected).
- **REASONED-REJECTED in v3.11.1-rc.1 (the Cursor v3.11.0-STABLE "SCHEMA-validate-then-write-gap" LOWs — do NOT re-flag):** `create_note`/`append_to_note` `content`/`frontmatter`/`separator` are UNCAPPED at the MCP schema boundary, but this is **deliberate**, not a gap. The sink is strictly LINEAR (`composeNote` → `writeNote`/`appendNote`: one `stringifyFrontmatter` + one `Buffer.byteLength`, NO parser/regex/per-note/whole-vault scan), already double-bounded (serve-http `deriveHttpBodyCap` = max(4 MB, maxFileBytes×1.5) = 7.5 MB BEFORE the handler, then `maxFileBytes` = 5 MB at the sink). The `parser-input-cap-invariant` deliberately scopes caps to SUPERLINEAR inputs; a 1 MB content cap (parity with `validate_note_proposal`, whose 1 MB exists for ITS superlinear wikilink scan) would sit BELOW the real 5 MB write limit and reject legitimate large notes (a regression). If you re-raise this, you MUST show a SUPERLINEAR cost or a sub-7.5 MB harm, not just "the schema lacks `.max()`".
