...
> Public roadmap for **enquire-mcp**, the long-term-memory MCP server backed by your local Obsidian vault. Updated 2026-07-24 (v3.12.0-rc.1 candidate: source-preserving doctor, package-coherent activation and hardened evaluation; earlier: the 2026-05 Tier-1 hardening sprint).
>
...
- **Process maturity** — 1669 tests, 9 release-required CI checks (7 currently branch-protected), semver-bound public surface, signed npm build provenance (SLSA Build L2), 12 state-driven OIA drift checks, structural invariant suite.
...
**The single highest-leverage move is publishing a reproducible LongMemEval retrieval score.** _(Correction, 2026-07-16 external audit: the earlier "no Obsidian MCP has any published number" premise is **no longer true** — `flowing-abyss/obsidian-hybrid-search` now publishes LongMemEval-S nDCG@5 0.895 / MRR 0.920 / Hit@1 0.889 / Recall@10 0.950 with a result JSON + reproduction. So the goal is no longer "be first" but "publish a fair, apples-to-apples comparison" against that peer's public protocol + our own ablations.)_ A published number moves us from the "Obsidian plugin" tier into the "serious memory infrastructure" peer tier (Zep/Mem0), and it plays to our strongest suit (retrieval). Second: own the precise message **"the only local memory layer grounded in *your* knowledge — not extracted and re-stored from conversations"** (mem0/Zep/Letta all extract; our vault *is* the source of truth — structurally simpler and privacy-superior).
...
- [x] **Messaging reposition** — SHIPPED: "grounded, not extracted" / "the only local memory layer grounded in your own knowledge" is present across README + `docs/COMPARISON.md`. _(2026-07-16 audit truth-reset. Next iteration is not "repeat the phrase across more files" but A/B a simpler outcome-led hero — the phrase is a proof point, not the headline.)_
