...
| `obsidian_ocr_pdf` | read | always | Tesseract OCR for image-only / scanned PDFs (multilingual via `lang`; process-wide serialized admission + finite per-call timeout). |
...
| `--watch`              | off     | Watch the vault for `.md` add/change/unlink events (and `.pdf` if `--include-pdfs`). On change: invalidate the parsed-note cache for that file; if `--persistent-index` is also enabled, incrementally re-sync just that file's FTS5 chunks AND (since v3.8.0 for `.md`, v3.8.0 for `.pdf`) the embed-db rows when the embed-db file exists. **v3.12.0-rc.27 `@rc` preview:** each ordinary add/change captures one path generation, stages Markdown or PDF lexical+semantic work from one source read, revalidates once after all awaited preparation, retries one drift once, then commits FTS5 → EmbedDb → HNSW without yielding. Fatal staging/embedding failure keeps the previous generation; optional OCR failure instead publishes the coherent PDF.js/empty generation and clears stale semantic rows. A sink-mutation failure quarantines semantic routing until restart; HNSW-only uncertainty falls back to EmbedDb and disables sidecar persistence. Deliberate same-account A → B → A path swaps require a future fd-bound protocol. **Hardlink convergence in the same preview:** an event for one regular-file hardlink inventories and independently refreshes every discovered admitted in-vault alias while preserving each exact searchable path. Every missing origin inventories even after the identity-only seed; unavailable identity uses one serialized, 50,000-path-count-capped global reconciliation. Distinct case/NFC/NFD paths never fold, and symlink/junction escapes remain excluded. **Startup in the same preview:** watched embedding capability is frozen for one server generation, changes observed before late sinks attach are replayed from final disk state, and a process-restart interlock prevents later admission after interrupted guarded startup. A stranded interlock requires the strict `clear-embeddings` recovery documented below. Editor saves are debounced via chokidar's `awaitWriteFinish`. `--exclude-glob` patterns are honored — edits to excluded paths don't fire. Off by default; opt in for long-running servers. |
...
| `--no-hnsw-persist`    | persist on | v2.16.0 — disable HNSW persistence sidecar. Default behavior: save `.hnsw.bin` + `.meta.json` next to `.embed.db`; reload it on the next serve when the embed-db signature matches. **In the v3.12.0-rc.27 `@rc` preview:** an uncertain watcher HNSW sync also suppresses saving automatically, leaving restart signature validation/rebuild as the recovery path. |
...
| `doctor` (**v3.12.0-rc.27 `@rc` preview**; command since v2.11.0) | `--vault <path>` `[--tier <basic\|hybrid\|hybrid-live>]` `[--index-file <path>]` `[--embed-file <path>]` `[--exclude-glob <pattern...>]` `[--read-paths <pattern...>]` `[--json]` | Source-state-preserving, tier-aware diagnostic (default `hybrid`). Every tier requires an accessible vault and no stranded default watcher-startup interlock; a selected custom `--embed-file` interlock is additionally checked as advisory for `basic` and required for both hybrid tiers. `hybrid` additionally requires SQLite + transformers + HNSW, valid FTS5/embed DBs, and exact non-empty q8 embedder/reranker cache artifacts; `hybrid-live` additionally requires PDF.js. OCR is advisory. SQLite is inspected from an in-memory byte snapshot, never through a source-path SQLite handle; active WAL/journal, unstable files, or files over 256 MB become `unverified`, not “missing.” JSON includes `tier`, `scope`, `limitations`, and per-check `required`; exit 0 means every required check is `ok`. READY is structural/runtime-only: it does not certify freshness, complete PDF coverage, watcher delivery, OCR packs, model artifact integrity/inference loadability, or at-rest privacy purge/index membership. Privacy flags affect the live vault counts only. Ordinary reads may update OS access-time metadata. |
| `setup` (v2.11.0) | `--vault <path>` `[--embedding-model <alias>]` `[--include-pdfs]` `[--skip-embeddings]` | Idempotently caches the selected q8 embedder and builds/refreshes FTS5 + embed-db. **v3.12.0-rc.27 `@rc` preview:** for the generated exact hybrid tier, separately run `enquire-mcp install-model rerank-bge`; use `setup --include-pdfs` for hybrid-live. |
| `configure` (v3.11.6) | `--vault <path>` `[--client <name>]` `[--tier <basic\|hybrid\|hybrid-live>]` `[--name <name>]` `[--http]` | Print a ready-to-paste MCP client config for this vault — **non-destructive** (writes nothing). Emits the snippet + where it goes for Claude Code, Claude Desktop, Cursor, Windsurf, VS Code, Codex, or a remote HTTP client (omit `--client` for all). **v3.12.0-rc.27 `@rc` preview:** VS Code receives its official encoded review/install URI, Claude Code and Codex receive copy-and-run commands, and Marketplace/Registry-only clients are explicitly labeled copy-only; every route retains the generated config fallback. CLI output pins the physical Node/package entrypoint; `--tier basic` is a live scan, `hybrid` adds FTS5 + embeddings + reranker + HNSW, and `hybrid-live` adds PDFs + `--watch`; the preflight includes tier-matched setup, reranker caching and doctor verification. |
| `first-run` (**v3.12.0-rc.27 `@rc` preview**) | `--vault <path>` `[--client <name>]` `[--tier <basic\|hybrid\|hybrid-live>]` `[--name <name>]` `[--http]` `[--embedding-model <alias>]` `[--quantize-embeddings <mode>]` `[--exclude-glob <pattern...>]` `[--read-paths <pattern...>]` `[--apply]` | Package-coherent configure → prepare → doctor orchestration. **Preview is the default:** only non-destructive `configure` runs; the exact remaining commands and their effects are printed. `--apply` explicitly authorizes index/model-cache preparation, then hybrid tiers run idempotent `setup`, cache the verified `rerank-bge`, and run tier-aware `doctor`; basic skips all index/model work. Child commands use raw argument vectors against the same physical package entrypoint, stop at the first failure, preserve privacy/model/quantization choices, and print an idempotent resume command. |
| `eval` (v2.12.0) | `--vault <path>` `--queries <jsonl>` `[--k <n>]` `[--matrix]` `[--reranker]` `[--persistent-index]` `[--per-query]` `[--json]` `[--output <file>]` | Built-in retrieval-quality benchmark harness. Reads a JSONL file of queries with known-relevant doc paths (`{query, relevant: ["path1", ...], id?, category?}`), runs `obsidian_search` for each, computes **NDCG@K + Recall@K + MRR + Hit@1/Hit@K + AllRel@K** + per-query latency, and (v3.11.6) **groups the same metrics by `category`** with per-query `missed_paths`/`top_paths`. Pretty table by default; `--json` for machine output; `--matrix` runs a 2x2 (graph_boost ± reranker) comparison. **v3.12.0-rc.27 `@rc` preview:** `--output <file>` produces input for `enquire-mcp eval-compare`; degraded matrix rows are invalid, and comparison requires zero retrieval errors plus identical `k`, query count and canonical cohort fingerprint. `|Δ|≥0.01` is a material-effect CI threshold, not statistical significance. See [`docs/EVALUATION.md`](EVALUATION.md). |
| `eval-compare` (**v3.12.0-rc.27 `@rc` preview**) | `<baseline.json> <after.json>` `[--json]` | Packaged A/B comparator for `eval --output` results. Requires a clean, finite result pair with the same `k`, query count and canonical cohort fingerprint; preserves the source-checkout wrapper's matrix behavior by comparing the first row of each matrix output. Returns nonzero for malformed/mismatched/errored inputs or when any tracked aggregate metric regresses by the fixed `|Δ|≥0.01` material-effect threshold. This is a CI heuristic, not statistical significance. |
...
**v2.16.0 — HNSW persistence:** when `--use-hnsw` is passed, the index is persisted to a sidecar `.hnsw.bin` + `.hnsw.meta.json` next to `.embed.db` after the first build. Subsequent serve starts load the sidecar when the embed-db signature matches; rebuild happens automatically on signature mismatch. Pass `--no-hnsw-persist` to disable. In the v3.12.0-rc.27 `@rc` preview, an uncertain watcher embedding-to-HNSW sync quarantines the live HNSW route, falls back to EmbedDb and permanently suppresses persistence for that server generation, so restart validation/rebuild is used instead of saving or querying a potentially partial graph.
...
| `prune` (v3.10.0) | `--vault <path>` `[--yes]` | GC cached index artifacts for OTHER vaults, keeping only the named vault's (`clear-cache`/`clear-index` only target the current vault). **Dry-run by default** — pass `--yes` to actually delete. Only ever removes enquire's own `<hash>.{json,fts5.db,embed.db,hnsw.bin,hnsw.meta.json}` files (incl. the `.json` parse cache holding full note bodies + `.tmp`/WAL sidecars). |
...
**Performance:** this path performs an O(n) in-memory cosine scan, so latency depends on corpus size, text shape, and hardware. For larger vaults, prefer `--persistent-index` + `obsidian_full_text_search` for indexed lexical retrieval, and use `obsidian_semantic_search` when BM25 misses. Measure your own corpus before setting an SLO.
