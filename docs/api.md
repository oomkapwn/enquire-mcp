...
| `--ocr-pdfs`           | off     | **v3.9.0-rc.1** — when paired with `--watch` + `--include-pdfs`, runs Tesseract OCR on image-only / scanned PDFs that pdfjs can't read text from. OCR-derived text feeds the embed-db so scanned PDFs stay in sync during long sessions. Requires `tesseract.js` + `@napi-rs/canvas` optional deps + the requested language pack downloaded manually from https://github.com/tesseract-ocr/tessdata_fast (no runtime CDN download — see [SECURITY.md "OCR network posture"](../SECURITY.md#ocr-network-posture)). |
