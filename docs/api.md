...
> **Channels:** stable since v3.8.x (`@latest` on npm) ships 44 tools including `obsidian_search` (hybrid BM25 + TF-IDF + ML embeddings, RRF-fused) with optional BGE cross-encoder reranking, `obsidian_embeddings_search`, `obsidian_hyde_search`, plus the `install-model` / `build-embeddings` / `clear-embeddings` / `setup` / `doctor` / `eval` subcommands. The `@rc` dist-tag carries the most recent release candidate — **currently v3.9.0-rc.3**, which adds OCR'd PDF watcher embed-sync (rc.1: `--ocr-pdfs`/`--ocr-langs`/`--ocr-max-pages` flags), HNSW in-memory live update (rc.2: search reflects vault edits within ~250ms; pre-3.9.0 needed serve restart), and R-10 adaptive HNSW refill (rc.3: closes the >66% excluded under-return). Benchmarks live behind `npm run bench:retrieval` (not a CLI subcommand). This document covers the **stable v3.8.x** surface plus the in-flight v3.9.0 RCs — see [CHANGELOG.md](../CHANGELOG.md) for full release history.
...
| `--ocr-pdfs`           | off     | **v3.9.0-rc.1** — when paired with `--watch` + `--include-pdfs`, runs Tesseract OCR on image-only / scanned PDFs that pdfjs can't read text from. OCR-derived text feeds the embed-db so scanned PDFs stay in sync during long sessions. Requires `tesseract.js` + `@napi-rs/canvas` optional deps + the language pack pre-cached (run OCR once online to populate `tessdata/`, then copy to the offline host — see [SECURITY.md "OCR network posture"](../SECURITY.md#ocr-network-posture) for the canonical procedure; no runtime CDN download during serve). |
...
| `--enable-reranker`    | off     | v2.9.0 — BGE cross-encoder reranking on top of RRF. ~30–50ms per query on M1; typical +5–10 NDCG@10 boost. Requires `@huggingface/transformers` optional dep. |
