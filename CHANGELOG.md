...
**Patch — round-16 self-audit batch.**

### Findings

- **F1 — v3.7.13 silent overclaim #6** (`src/tools/write.ts:145-148`). The v3.7.13 M1 fix reordered `renameNote()`'s write steps (rename first, backlinks second) and added a long comment block inside the function explaining the new order. But the function-LEVEL TSDoc header (lines 145-148, what TypeDoc publishes + IDE hover surfaces) still described the OLD ordering: *"backlink-bearing files first, source last, fs.rename last — a mid-operation crash leaves backlinks pointing at the still-present old name"*. That's the exact (LYING) comment v3.7.13 M1 replaced inside the function body — but the public-facing docstring kept it. **6th overclaim instance** documented (after v3.6.1 CRIT-1, v3.6.2 K-1 callsite count, v3.6.4 K-1 cli.ts callsites, v3.7.10 silent `files`-array miss, v3.7.11 round-13 retroactive correction). Header now matches reality + cross-references v3.7.13 M1.
- **F2 — `vault.renameFile` had the M2 TOCTOU race for the destination** (`src/vault.ts:633-688`). v3.7.13 M2 fixed `writeNote(overwrite=false)`'s stat-then-write race by switching to the `wx` open flag, but `renameFile(overwrite=false)` had the SAME pattern: `stat(toAbs)` → check ENOENT → `fs.rename(fromAbs, toAbs)`. POSIX `rename(2)` silently replaces the destination if it exists, so a parallel writer creating the destination between the stat and the rename would get its file clobbered by ours, bypassing the `overwrite=false` guard. Fix: use `fs.link(fromAbs, toAbs)` for the non-overwrite path. `link(2)` fails atomically with `EEXIST` when the destination exists — no stat-then-act window. After successful link, `fs.unlink(fromAbs)` completes the move semantic. `EXDEV` (cross-device link) falls back to `copyFile(..., COPYFILE_EXCL)` + `unlink` for the same atomic guarantee on bind-mounted vaults. **+1 negative-control test** firing two simultaneous `renameFile()` calls at the same destination and asserting exactly-one-success.
- **F3 — `appendNote` stat-then-append size-cap bypass** (`src/vault.ts:697-704`). Pre-3.7.14 the size guard was `fs.stat(abs)` → check `before.size + addition <= maxFileBytes` → `fs.appendFile(abs, addition)`. Under parallel appends, the stat could report size X, another process appended size Y between stat and our append, and our append took the file to X+Y+addition — past the cap. Fix: open the file via `fs.open(abs, "a")`, `handle.stat()` from the open fd to get the current size, check the cap, write via `handle.write()`, close in `finally`. Keeps the stat→write window inside a single kernel-held fd, and the "a" mode flag's POSIX append guarantee ensures atomic end-of-file writes.
- **F4 — "8 required CI gates" claim ungated** (`README.md:115`, `README.md:185`, `CLAUDE.md:29`). v3.7.13 M5 bumped the claim from 7 → 8 manually, but no test gated it against `.github/workflows/release.yml`'s actual `REQUIRED` regex. Recurrence of the v3.5.9 anti-pattern ("Hardcoded counts in docs without an invariant"). Fix: new docs-consistency invariant `'N required CI gates' claims match release.yml REQUIRED regex count` that (a) parses the `REQUIRED="lint|test \(22\)|..."` assignment, splits on `|`, counts entries; (b) cross-checks against `REQ_COUNT=N` in the same workflow (double-source-of-truth structural guard); (c) asserts every `**N**? required` claim in README + CLAUDE.md matches the actual count. If a 9th gate is added later, all three drift surfaces fail together.
- **F5 — Automated `gh release create` in release workflow** (`.github/workflows/release.yml:124-167`, M13 from round-15). Pre-3.7.14 README claimed *"npm + GitHub release per tag"* but `gh release create` was a manual post-release step (CONTRIBUTING.md:87). Originally deferred to v3.8.0 in round-15; landed early because it's trivial. Workflow now: extracts the matching `## [X.Y.Z]` section from `CHANGELOG.md` via `awk`, marks stable versions (`latest` dist-tag) as Latest with `--latest --target main`, marks prereleases with `--prerelease`, falls back to a CHANGELOG link if the section can't be parsed, skips with a warning if a release already exists (manual creation before the tag-push). CONTRIBUTING.md updated to reflect the new automation.

### Stats

- **815 tests** (was 813 in v3.7.13). **+2 tests**: 1 F2 negative-control (`tests/write.test.ts`) + 1 F4 invariant (`tests/docs-consistency.test.ts`).
- **+1 docs-consistency invariant** (required-gates count).
- npm tarball unchanged; v3.7.14 is a pure correctness + automation patch.

### Method

Round-16 was a SELF-audit (no external auditor), but used the round-15 fix patterns AS the audit methodology — apply each fix's class pattern across the rest of the codebase. The result: 2 real new bugs (F2/F3) that v3.7.13's external-audit-driven scope didn't catch, plus 1 silent overclaim instance (F1) of the SAME class that recurred in v3.7.10, v3.7.11, v3.7.12. **Class-pattern sweeps after every external-audit cycle is the right post-ship discipline** — externals find the entry point, the class sweep cleans up the siblings before another auditor flags them.

## [3.7.13] — 2026-05-18

> **TL;DR:** Round-15 external audit response (4th independent external audit since v3.6.0). 15 ship-ready fixes landed: 3 High (PDF page slicing pre-extraction DoS vector, HTTP stateful pre-initialize resource leak, `engines.node` floor mismatch with CI matrix), 9 Medium (renameNote rollback recovery, atomic non-overwrite create via `wx`, OCR lang regex validation, README required-gates count, per-file coverage fail-on-missing, parse-cache chmod-only-newly-created, broken docs/api.md → SECURITY.md link, benchmarks latency-column de-duplication, COMPARISON.md test-count gate), 3 Low (OCR/read_pdf page-range refine, github-metadata invariant fail-loud in CI with GH_TOKEN, exclude `docs/audits/` from npm package). **+11 tests** (813 total), with negative-controls on every new invariant. 2 audit findings already closed by v3.7.12 (L1 `.base` path normalization, L3 canvas mtimeMs leak). 7 architectural findings deferred to v3.8.0 with reasoning (M4/M6/M7/M13/L5/L6 — write-tool smoke variant, reranker-smoke-in-CI, release automation, declarations-vs-runs test count).

**Patch — round-15 external audit batch.**

### High-severity fixes

- **H1 — PDF page slicing applied AFTER full document extraction** (`src/pdf.ts` + `src/tools/media.ts`). Pre-3.7.13 `extractPdfText(buf)` iterated EVERY page of the doc, then `readPdf` sliced down to the caller's `args.pages` window. Wasted CPU/memory on big PDFs and a bearer-token DoS vector in `serve-http` (a client with a valid token could request small page ranges of 1000-page PDFs to peg the server). Fixed by adding `ExtractPdfTextOptions.pageRange` and threading the requested window through to `doc.getPage()`, so only the needed pages get deserialized. `result.pageCount` still reports the document total (read from pdfjs metadata, not from page count returned), so callers can paginate. **+4 tests** in `tests/pdf.test.ts` including negative-control that omitting `pageRange` extracts every page.
- **H2 — HTTP stateful mode allocated server/transport BEFORE checking that the body was `initialize`** (`src/http-transport.ts:480-516`). Any POST without `Mcp-Session-Id` allocated a fresh `McpServer` + `StreamableHTTPServerTransport` pair, then handed the body to the SDK; in some failure modes (the body not being a valid `initialize` request), `onsessioninitialized` never fired and cleanup never ran, leaking the pair. Bearer-token DoS — not anonymous, but easy for any client with a valid token to fire create-and-abandon traffic until `maxSessions` blocks further work. Fix: new `isInitializeRequest(body)` helper validates the JSON-RPC method BEFORE allocation; non-initialize POSTs are rejected with `400 -32600`. **+5 tests** in `tests/http-transport.test.ts` covering single + batch + 5 negative-control cases (tools/list, tools/call, malformed, batch-without-initialize, case-sensitivity).
- **H3 — `package.json#engines.node: ">=20"` was wider than the CI matrix (Node 22+24)** (`package.json:168-170`, `docs/QUICKSTART.md`). Pre-3.7.13, users on Node 20 could install the package (npm honored the loose `>=20`) but get into trouble at runtime — `pdfjs-dist@5.7+` requires `>=22.13.0`, so PDF features either crashed or silently no-op'd. CI hasn't tested Node 20 since v3.5.11, but the engines floor stayed at the old value. Fixed: bumped `engines.node` to `>=22.13.0` matching the CI matrix. Users on Node 20 now get a clean `unsupported engine` warning at install time. QUICKSTART updated to recommend pinning v3.7.12 for Node 20 use cases.

### Medium-severity fixes

- **M1 — `renameNote()` write order made backlinks inconsistent on partial failure** (`src/tools/write.ts:263-277`). Pre-3.7.13: (1) rewrite all backlink-bearing files → (2) rewrite source content at OLD path → (3) `fs.rename` OLD → NEW. If step 3 failed, backlinks were ALREADY pointing at the NEW name (rewritten in step 1) but the file was still at the OLD path — backlinks reference a phantom destination. The inline comment claimed "backlinks pointing at the still-present old name (worst case: safe, recoverable)" — but that was FALSE; step 1 had already broken them. Fix: reordered to (1) rewrite source at OLD path → (2) `fs.rename` (atomic, most-failure-prone step runs FIRST) → (3) rewrite backlinks (destination exists, so updated backlinks always resolve). Every failure mode is now recoverable by re-running the same call (each step is idempotent on re-input).
- **M2 — `overwrite=false` create path was not atomic** (`src/vault.ts:564-580`). Pre-3.7.13 `writeNote(..., overwrite=false)` was `stat()`-then-`writeFile()`: between the two calls, another process or parallel MCP call could create the file, and overwrite=false would silently clobber it. Fixed by using `fs.writeFile(abs, content, { flag: "wx" })` for the non-overwrite path — the kernel atomically refuses to open the file if it exists, translating to the same user-facing "Note already exists" error. **+1 integration test** firing two simultaneous `createNote()` calls against the same path and asserting exactly-one-success.
- **M3 — OCR `lang` was an unconstrained string** (`src/tool-registry.ts:731`). Schema accepted any string and passed it straight to `tesseract.createWorker(langs, ...)`, which downloads each requested language pack on first use (~10 MB each). A bearer-token client could request `lang: "eng+rus+jpn+chi_sim+...+ben+hin+kor+vie+tha"` to trigger N parallel pack downloads + slow multi-language OCR. Resource exhaustion DoS. Fix: schema regex `/^[a-z]{3}(_[a-z]+)?(\+[a-z]{3}(_[a-z]+)?){0,7}$/` (1-8 `+`-joined Tesseract codes, lowercase 3-letter codes with optional `_variant` suffix for regional packs like `chi_sim`, `chi_tra`). The pattern matches the standard Tesseract trained-data file naming.
- **M5 — README claimed "7 required" CI gates, but release.yml requires 8** (`README.md:115` + `README.md:185` + `CLAUDE.md:29`). The 8th gate (`docs` — TypeDoc generation) was added in v3.7.10's release.yml but the README + CLAUDE.md text wasn't updated. The release workflow already enforces 8 (`REQ_COUNT=8`), so the public-facing claim was simply stale. Fixed: 7 → 8 across all 3 surfaces with v3.7.10 attribution.
- **M8 — `scripts/check-per-file-coverage.mjs` warning-only on missing coverage entries** (`scripts/check-per-file-coverage.mjs:79-93`). If a `FLOORS` key didn't have a matching coverage entry (e.g., file renamed, vitest include-regex change excluded it), the script emitted a `WARN` and continued. So a per-file floor could silently disappear from enforcement. Fixed: `hasError = true` + clear "update FLOORS or coverage config" message; the script now exits 1 on missing entries.
- **M9 — `saveDiskCache()` chmod'd the parse-cache parent dir even if it pre-existed** (`src/vault.ts:361-367`). Pre-3.7.13, `await fs.chmod(cacheDir, 0o700)` ran unconditionally — clobbering perms on a custom `--cache-file` parent (e.g., a shared Dropbox folder, an NFS mount with group-readable defaults). `FtsIndex` and `EmbedDb` already use the `parentExisted` gate (only chmod if newly-created); applying the same pattern here closes the inconsistency. Fix: `stat(cacheDir).then(true).catch(false)` → if parent existed, skip the chmod; if we created it, lock perms.
- **M10 — `docs/api.md` linked to a non-existent README anchor** (`docs/api.md:80`). The `--persistent-cache` row referenced `[Cache & privacy](../README.md#cache--privacy)` but README has no such section — the canonical content lives at `SECURITY.md#persistent-cache-privacy-posture`. Users following the link missed the privacy warning. Fix: redirect to the real anchor in `SECURITY.md`.
- **M11 — Benchmark latency numbers contradicted between TL;DR and stack-config tables** (`docs/benchmarks.md:14-23` vs `:107-115`). TL;DR was re-synced in v3.7.10 (round-12 audit M-5) to A18 Pro measurements; the stack-config implementation table still carried the original M1 numbers (~110ms / ~230ms / ~520ms / ~730ms vs the new 179 / 401 / 1028 / 1344). Fix: dropped the latency column from the implementation table entirely. Latency now has a single source of truth (TL;DR, re-measured from `bench/benchmarks.json`).
- **M12 — `docs/COMPARISON.md` test count was stale + ungated** (`docs/COMPARISON.md:46`). Pre-3.7.13 COMPARISON said `Test count (public) | **786**` while README+package were at 787 (and 801 after v3.7.12). The previous COMPARISON docs-consistency gate covered tools+prompts but missed test count. Fix: bumped to 813 + added a new invariant that locks "**N**" in the `Test count` row against the actual `countActualTests()` result.

### Low-severity fixes

- **L2 — Invalid OCR page ranges silently returned empty success** (`src/ocr.ts:166-185`, `src/tool-registry.ts:739`). `pages: [10, 5]` made `from=10, to=5`, the loop body never executed, and the response was a successful `{ pages: [], full_text: "" }`. Clients couldn't distinguish "PDF has nothing on those pages" from "your range was invalid". Fix: schema-level `.refine(([from, to]) => to >= from)` rejects inverted ranges at the tool boundary; the library function still clamps as a defense-in-depth. Same constraint added to `obsidian_read_pdf` for parity.
- **L4 — `tests/github-metadata-invariant.test.ts` silently no-op'd in CI** (`tests/github-metadata-invariant.test.ts:100-127`, `.github/workflows/ci.yml`). Pre-3.7.13 the test had `if (!available) return;` (early-return without assertion) when `gh` wasn't authenticated AND `if (!meta) return;` (early-return) when `gh` was available but the API call failed (rate limit, network). Both branches let CI count the test as "passed" without verifying anything. Fix: (1) CI workflow now passes `GH_TOKEN: ${{ github.token }}` so `gh api` always works against the live repo, (2) test now fails loud with a clear error message when `gh` is available but the API fails (rate limit / network / token scope problem worth surfacing).
- **L7 — npm package shipped `docs/audits/` (228 KB internal audit process notes)** (`package.json#files`). Pre-3.7.13 `"files": ["dist", "docs", ...]` shipped the entire `docs/` tree. End users don't need rc-stage audit plans or `findings/L9-process.md` snapshots. Fix: explicit file list (`docs/api.md` + QUICKSTART/COMPARISON/benchmarks/http-transport.md + `docs/api-reference/`), dropping the `docs/audits/` subtree from npm tarballs.

### Audit findings already closed by v3.7.12

- **L1 — `.base` path docs say extension optional, implementation doesn't.** Closed by v3.7.12 H2 (`.base` path normalization in `readBase` mirrors `readCanvas` / `readPdf`).
- **L3 — Malformed canvas summaries report `mtimeMs` as `size_bytes`.** Closed by v3.7.12 M3 (`let size = 0` initialization in `listCanvases`).

### Architectural findings DEFERRED to v3.8.0

Documented inline so future audits don't re-flag:

- **M4** — HTTP transport `serve-http` doesn't accept `--enable-reranker` / `--use-hnsw` / `--include-pdfs`; docs claim "identical semantics" for those flags. Capability matrix is a docs-only fix but the underlying flag plumbing needs design work (HNSW persistence in HTTP mode, reranker model lifecycle across stateless sessions).
- **M6** — Real reranker smoke not enforced in CI. Adding `ENQUIRE_LOAD_RERANKER_SMOKE=1` to a CI job will download ~110 MB of BGE weights on every PR; needs a dedicated job with cache strategy.
- **M7** — Write-tool MCP registration + `--enabled-tools` / `--disabled-tools` filtering not smoke-tested. Requires a new smoke variant — non-trivial design.
- **M13** — README claims "npm + GitHub release per tag" but `gh release create` is manual per CONTRIBUTING.md. Automating this in `release.yml` is on the v3.8.0 backlog.
- **L5** — Public test count is `it()` declaration count, not Vitest-actually-ran count. Switching to Vitest JSON output is a meaningful design change to the count invariant.
- **L6** — Watcher tests timing-sensitive (chokidar polling). Refactoring to observable events / injectable watcher adapter is a multi-day test refactor.

### Stats

- **813 tests** (was 801 in v3.7.12). **+12 tests**: 4 pageRange (`tests/pdf.test.ts`), 5 isInitializeRequest (`tests/http-transport.test.ts`), 1 atomic create-race (`tests/write.test.ts`), 1 COMPARISON test-count gate + 1 STABILITY-exports invariant alignment (`tests/docs-consistency.test.ts`).
- **+1 docs-consistency invariant** (COMPARISON.md test count).
- **CI**: GH_TOKEN wired to test jobs so `tests/github-metadata-invariant.test.ts` actually runs against the live repo.
- **npm package**: `docs/audits/` (228 KB) removed from published tarball.

### Method

Round-15 audit found findings that 4 prior internal audits + 3 prior external audits all missed. Notably H1 (PDF page slicing) and H2 (HTTP stateful pre-check) are CPU/memory DoS vectors that became obvious only when the auditor traced the resource path end-to-end — a methodology different from the K-1 destructive-bootstrap class that previous audits emphasized. Re-confirms the v3.6.1 method note: **every minor / major needs ≥2 independent external auditors with DIFFERENT methodologies**.
