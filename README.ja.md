...
**46 ツール · 19 MCP プロンプト · 2212+ ユニットテスト · 50+ 言語 · v3.11.x 安定版 · semver 準拠 · MIT · npm ビルドプロベナンス（SLSA L2）。**
...
| **プロダクションの信頼性** | ✅ 既定で read-only、privacy filter、認証 HTTP、semver 契約、2212 tests、13 release gates、SLSA L2 provenance |
...
[GitHub Release `v4.0.0-rc.4`](https://github.com/oomkapwn/enquire-mcp/releases/tag/v4.0.0-rc.4) では `enquire-mcp-basic-4.0.0-rc.4.mcpb` と checksum、inventory、SBOM、notices、provenance を提供します。バンドルにはサーバー JavaScript と通常の依存関係が含まれ、対応 MCPB ホスト側で Node.js 22.13 以降を提供する必要があります。
...
| **2212 ユニットテスト · リリース必須 CI チェック 13 個 · 現在ブランチ保護対象は全 13 個** | 現在確認済みのリリース状態。運用詳細は下に固定しています。 |
