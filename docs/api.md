...
| `obsidian_search` | read | always | Hybrid retrieval — BM25 + TF-IDF + embeddings fused via RRF (v2.0 default). v3.11.6-rc.7 adds optional `queries: string[]` (multi-query fan-out, max 8) — each phrasing is searched and the result lists are RRF-merged with the main `query`, so a note matching ANY phrasing floats up (complements HyDE). v3.11.6 (S-5) adds optional `explain: true` — attach a per-hit `explain` object (RRF rank/score, wikilink graph-boost `in_degree`/`score_delta`, reranker score, and the opt-in `--recency-weight`/`--feedback-weight` `rank_before→rank_after` movement) so you can see WHY each hit ranked where it did and verify the recency/feedback re-ranks actually change the order. Single-query only; off ⇒ no field. |
