...
| `configure` (v3.11.6) | `--vault <path>` `[--client <name>]` `[--tier <basic\|hybrid\|hybrid-live>]` `[--name <name>]` `[--http]` | Print a ready-to-paste MCP client config for this vault — **non-destructive** (writes nothing). Emits the exact snippet + where it goes for Claude Code, Claude Desktop, Cursor, Windsurf, VS Code, Codex, or a remote HTTP client (omit `--client` for all). `--tier basic` = live scan (no `setup` needed); `hybrid` (default) = FTS5 + reranker + HNSW; `hybrid-live` adds PDFs + `--watch`. Closes the "installed but not wired up" activation gap. |
