...
for (( page_attempt=1; page_attempt<=2; page_attempt++ )); do
  ...
  it("keeps release.yml wired to the shared evaluator and an exact mirrored inventory", () => {
    ...
    expect(releaseMutationInventoryProblems(extraProjectMutation)).toContain(
      "release mutation hybrid inventory expected 538 first / 22 all, found 539 first / 22 all (legacy 475/19; declarative 64/3; cases 66)"
    );
    ...
    expect(releaseMutationInventoryProblems(projectModeDrift)).toContain(
      "release mutation hybrid inventory expected 538 first / 22 all, found 537 first / 23 all (legacy 473/20; declarative 64/3; cases 66)"
    );
    ...
    expect(releaseMutationInventoryProblems(legacyFreeMatrix)).toContain(
      "release mutation final closed graph expected 560 unique descriptors / 536 cases and roots / 541 expectations / 24 dependency-only, found 67 descriptors / 66 cases / 66 roots / 66 expectations / 1 dependency-only"
    );
    ...
    expect(nestedStraightLineProblems).toContain(
      "release mutation hybrid inventory expected 538 first / 22 all, found 540 first / 22 all (legacy 476/19; declarative 64/3; cases 66)"
    );
    ...
    expect(releaseMutationPlan.caseExecutions).toBe(66);
    expect(releaseMutationPlan.expectationExecutions).toBe(66);
