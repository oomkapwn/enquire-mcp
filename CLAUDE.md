...
- **v3.8.0-rc.6 shipped (current)**: round-23 external audit response (6 fixes). T-FLAKE-1 vitest per-it timeout (build-embeddings CI flake), N-4 protobufjs GHSA-jggg-4jg4-v7c6, N-5 serve-http `--watch` help parity, ARCH-1 circular import (`buildEmbedText` → embed-pipeline.ts), R-4 contextPack hard budget cap, OIA promoted advisory → required (9th gate). **838 tests** (unchanged). No open audit items from rounds 1–23 as of 2026-05-21.
- **v3.8.0 backlog status**: All known audit findings from rounds 1–23 closed. Remaining v3.8.0 items: serve-http parity (P1-3), OCR concurrency/timeout, search underfill (P2-7), FTS5/embedding chunking parity (P2-8), stateful session race (P2-10), HTTP server close cleanup (P2-11), doctor privacy filters (P2-12), npm package broken links (P2-18), canonical CLI docs (P2-20), FTS gating wording (P3-21), heading tilde fences (P3-25), HNSW metadata validation (P3-27), setup-snippet mkdir (P3-29) + other v3.8.0 backlog originals. External audit before promotion to v3.8.0 stable.
