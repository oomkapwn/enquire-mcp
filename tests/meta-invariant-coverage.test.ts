...
describe("META-invariant: exact structural census + NEGATIVE control coverage", () => {
  ...
  // migration adds 12 bounded candidates for 65 total, and m111, paired m112-m113, m114-m158,
  ...
  beforeAll(async () => {
    ...
    // NEGATIVE control: append the forty-nine exact historical root call-node byte spans after
    ...
    expect(declarativeSealStart).toBeGreaterThan(m158BlockStart);
    ...
    // current topology has 30 frozen roots; wrapping multiline m159 reindents its
    // call-node bytes, invalidating the m159 identity, root, check, and matcher leaf
    // while leaving the shared matcher with exactly 29 bound owners.
    ...
    expect(sha256Text(discardedM159CallNode)).toBe("3ef1f1f8761208709fb242495395b32c5b7f757700f0e7bc6f74421e40c80547");
    const discardedProvenanceRoot = replaceExactly(
      matrixSource,
      `      ${legacyM159CallNode},`,
      ["      (", `        ${discardedM159CallNode},`, "        mcpbInputs.integrity", "      ),"].join("\n")
    );
    expect(preparedAudit.auditMatrix(discardedProvenanceRoot)).toEqual(
      expect.arrayContaining([
        expect.stringMatching(/release mutation hybrid current source must retain exact SHA-256/),
        expect.stringMatching(/release mutation hybrid current matrix slice must retain exact SHA-256/),
        expect.stringMatching(/release mutation hybrid legacy case release\.case\.m159 has no remaining root call/),
        expect.stringMatching(
          /release mutation hybrid shared primary matcher 84d0f85b5d5eb401cf80482bfec6ab0a2d4f60687d54d364f189bdc73052f680.*exact closed iterable\/runtime topology for 29 frozen root/
        )
      ])
    );
    ...
    // Registry loop and legacy m159, preserving the frozen global case order.
