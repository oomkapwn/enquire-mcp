# External Audit Request — enquire-mcp v3.11.0-rc.9 (FULL, from scratch)

**Status:** OPEN — commissioned for the **v3.11.0 → `@latest` promotion gate**.
**Date issued:** 2026-06-23
**Supersedes:** `AUDIT-REQUEST-FULL-FROM-SCRATCH-v3.11.0-rc.7-2026-06-23.md` (this brief reflects rc.8 + rc.9, which shipped after that one).
**Repository:** https://github.com/oomkapwn/enquire-mcp (public, MIT) · npm `@oomkapwn/enquire-mcp`
**Target of audit:** `@rc` = **3.11.0-rc.9**. Pin the exact commit before you start:
...
npm view @oomkapwn/enquire-mcp@rc version          # must read 3.11.0-rc.9
```

> The branch that introduced rc.9 was `audit/v3.11.0-rc.9-nfc-tag-class` (pre-squash commit `84ab480`); the authoritative target is the **squash-merge commit on `main`** tagged `v3.11.0-rc.9`. Grade THAT commit and cite its SHA in every finding.
...
A **comprehensive, adversarial, from-scratch audit of the entire project** — every `src/` module, every doc, every workflow/script/config — **with extra scrutiny on the recent changes** (the v3.11.0 line: rc.1 → rc.9, summarized in §6). Find real defects the internal apparatus is blind to.
...
2. **Adversarial re-verification of the maintainer's own recent verdicts** — especially the rc.9 downgrades (§6.5). We explicitly invite you to try to prove us wrong.
...
- **Scale:** 40 `src/*.ts` modules (~5,100 source lines), **46 MCP tools** (34 always-on read + 4 opt-in diagnostic + 7 write gated by `--enable-write` + 1 feedback gated by `--feedback-weight`), **19 MCP prompts**, 3 resources. **80 test files / 1350 canonical `it()`** (data-driven loops expand the runtime count higher); 19 `*-invariant.test.ts`; 16 `scripts/*.mjs`; 4 GitHub workflows.
...
- **Threat model (single-user, local vault):** the vault owner is trusted; the *agent/MCP client* is semi-trusted (could be driven by untrusted content the user pasted); on `serve-http`, a bearer-authenticated client is the adversary for DoS/leak. Note files themselves may contain adversarial content (prompt-injection text, pathological regex/markdown, hostile frontmatter).

Authoritative docs to read first: `README.md`, `CLAUDE.md` (the maintainer's North-Star + the running anti-pattern ledger — read this; it tells you the project's own list of recurring failure classes), `SECURITY.md`, `STABILITY.md` (the semver contract), `docs/api.md`, `CHANGELOG.md` (esp. the rc.1→rc.9 entries).
...
| `tool-registry.ts`, `tool-manifest.ts` | tool registration + gating (the single source of truth for the 46-tool count + `readOnlyHint`) |
...
| `name-fold.ts` | **canonical Unicode folders** `foldName`/`foldTag`/`nfcLower`/`nfc` (rc.9 added the tag/value folders) |
| `parser.ts` | frontmatter+body split, wikilink/embed/tag/heading extraction, `bodyStartLine` (line-number arithmetic) |
...
| `tools/read.ts` | always-on read tools (list/search/neighbors/stats/tags/chat-thread/frontmatter) |
...
| `tools/search.ts` | hybrid search orchestration, RRF, rerank, graph-boost, recency, `filter_frontmatter`, adaptive HNSW refill, privacy-filter terminal pruning |
| `tools/meta.ts` | `obsidian_open_questions` (ReDoS-sensitive; worker sink-bound), wikilink lint, tag suggest, paper audit, `findBestMatch` |
...
| `fts5.ts` | SQLite FTS5 index (`peekFtsMetaSafe` never-throw peek), tokenization, escaping |
...
2. **Runtime DoS / algorithmic complexity** — O(n²)/O(K×N) amplifiers, unbounded scans, ReDoS, OOM, on always-on **remotely-reachable** tools (`dataview_query`, `query_base`, `open_questions`, `read_canvas`, `validate_note_proposal`, `search`). The drift gates read text, not control-flow cost.
3. **Encoding correctness** — Unicode NFC/NFD on macOS APFS (names, tags, frontmatter keys+values), surrogate splitting, case-folding under the sink's actual flags. (rc.9 closed the tag surface; look for the *next* uncovered identity/encoding surface.)
...
5. **Claimed-guarantee vs. code-guard** — any "blocked"/"zero outbound"/"fails closed"/"never throws"/"SLSA L2"/"enforced"/"throws if" claim in SECURITY.md/TSDoc/README must point at a real guard. Find a claim with no enforcing code path.
...
8. **Test-theater** — tests that pass without exercising the code they claim to (silent `return` skips on security surfaces, vacuous assertions, invariants whose detector can't actually fire, NEGATIVE controls that are commented out/empty).
...
npm test                      # ~1350 it() (+ data-driven expansion)
...
All of the above are expected to pass on rc.9. If any fails on a clean checkout, that itself is a finding. Then audit beyond what these check.

---

## 6. Recent changes — audit these hardest (v3.11.0 line, rc.1 → rc.9)
...
Verify SHA-pins on all GitHub Actions are correct + comments match; no supply-chain regressions; `npm audit` clean.
...
### 6.5 rc.9 — EXTERNAL re-audit response + the NFC-tag class — **re-verify the maintainer's verdicts adversarially**
A prior external auditor reviewed the *older* rc.5 commit; the maintainer's agent re-verified each finding against HEAD. We want you to **independently re-check the agent's conclusions** — this is a check on the maintainer's own work:

- **L-TAG-1 (confirmed, fixed):** tag comparisons lacked NFC across ~13 sites → shared `foldTag`/`nfcLower`/`nfc` in `name-fold.ts` + producer NFC + the **name-fold inventory invariant extended with the tag signature** + behavioral NFD→NFC tests. **Verify:** is EVERY tag/identity comparison now NFC-folded? Is the extended invariant's detector actually non-vacuous and complete (does it catch the `#`-strip-then-lowercase shape in both orderings AND the bare `parsed.tags...toLowerCase()` shape)? Any remaining unfolded identity surface (frontmatter KEYS? other user-string lookups)?
- **T-MED-1 (maintainer downgraded MED → FALSE POSITIVE):** the agent claims `HnswIndex.applyDiff` + `watcher.syncHnswForFile` are fully synchronous, so cross-file watcher events cannot interleave (JS run-to-completion), and the auditor's proposed mutation queue is redundant. They pinned the property with `tests/hnsw-sync-critical-section.test.ts` (no `await` in the critical section). **Challenge this:** is the critical section truly await-free on every path (md/pdf/unlink)? Is there ANY interleave or lost-update window (e.g. between embed-db upsert and HNSW apply, or vs. close-time flush, or a second writer)? Is the "no-await" invariant the right guarantee, or is there a residual we missed?
- **I-COV-1 (mechanism rejected, sibling fixed):** the agent showed the spawned `dist/` child is NOT coverage-instrumented (vitest v8 uses the inspector, not `NODE_V8_COVERAGE`) — but bumped the coverage CI job `timeout-minutes` 5→10 as a real flake-risk. **Verify** the mechanism claim and whether any other CI job has an inadequate time budget vs its work.
- **I-HNSW-1 (downgraded INFO → WAI):** `capacity()` has zero production callers; the fallback now returns an honest `Infinity`. **Confirm** zero callers and that the orphaned `capacity()`/`resize()` API can't mislead a future caller.

### 6.6 Cross-cutting recent surfaces
9-language READMEs (rc.2/i18n — anchor integrity + per-language numeric claims), forgetting-aware staleness (rc.10 line), frontmatter-aware `filter_frontmatter` search.

---

## 7. Comprehensive coverage checklist (by class)

For each class: is it fully closed, or is there an uncovered sibling/surface? (The project's signature failure mode is "instance fixed, adjacent sibling missed.")

- [ ] **STRIDE / security:** auth (bearer `timingSafeEqual`, min-length), rate-limit, CORS (expose/allow headers), path traversal, symlink escape, input validation (zod `.max()` on every remotely-reachable string), injection (FTS5/SQL/DQL/glob), prototype pollution.
...
- [ ] **Unicode / NFC / encoding:** names, tags (rc.9), frontmatter keys + values, DQL/bases field names, surrogate-pair splitting in chunking/snippets, case-fold under `/i`/`/u`.
- [ ] **Concurrency:** every long-lived shared-mutable singleton — serialized or provably interleave-safe? (watcher HNSW/rowsByLabel/embed-db, FeedbackStore, SessionRegistry, module-level caches.)
- [ ] **Resource / DoS caps:** every always-on whole-vault scanner CAP-or-EXEMPT (`resource-bound-invariant`); per-request amplifiers on remote tools; canvas/PDF/OCR memory caps; HNSW growth.
...
- [ ] **Right-to-erasure / data-at-rest:** writers ⊆ erasers for every cache/sidecar/`.tmp`; no raw note text survives `prune`/`clear-*`; SECURITY.md content-at-rest claims true.
- [ ] **Write-fidelity / data-loss:** create/rename/append/replace/frontmatter_set atomicity + edge cases; line-number arithmetic (frontmatter offset); backlink-rewrite plan correctness; case-insensitive FS.
- [ ] **Claimed-guarantee vs code-guard:** "zero outbound in serve" (embeddings/reranker/OCR offline enforcement), SLSA L2, fail-closed `.base` predicates, `*Safe`/peek never-throw, "fails closed" privacy filter.
- [ ] **MCP contract:** `readOnlyHint` correctness (K-3: every fs/state mutator is in `KNOWN_WRITE_HANDLERS`), tool schemas, error shapes, stateful session lifecycle.
- [ ] **Supply-chain:** SHA-pinned actions + correct comments; `run:`-download content-pinning (mcp-publisher); `overrides` correctness; `check-audit.mjs` allowlist (currently empty) justified; phantom/undeclared deps; `files[]` accuracy.
- [ ] **Docs / claim-vs-reality:** counts (46 tools / 19 prompts / test count) across README ×9, llms.txt, AGENTS.md, STABILITY, COMPARISON, api.md, server.json, CITATION; version currency; CLI flag docs vs real `.option()`s; README anchor integrity; TSDoc-vs-impl drift.
- [ ] **Test/CI integrity:** no silent-skip on security surfaces (CI-GUARD pattern); no vacuous/test-theater assertions; every `*-invariant.test.ts` has a real NEGATIVE control (meta-invariant); coverage floors honest; flake-blocks-release risks.
- [ ] **Retrieval correctness:** RRF fusion, rerank application, graph-boost, recency blend, `filter_frontmatter`, chunking parity (FTS5 vs embeddings), HNSW under-return, eval metric correctness (no >1.0 inflation, dedupe).

---

## 8. Specific high-value questions we most want challenged

1. Can a **bearer-authenticated serve-http client** hang the event loop or exhaust memory through any always-on tool (`dataview_query`, `query_base`, `open_questions`, `search`, `read_canvas`, `validate_note_proposal`, `read_pdf`/`ocr_pdf`)? Provide a concrete repro if so.
2. Is the **T-MED-1 "synchronous critical section" claim** actually airtight (§6.5)? Find any interleave/lost-update/torn-state window in the watcher → embed-db → HNSW → rowsByLabel → disk-flush pipeline.
3. Is the **prototype-pollution fix** (rc.8) complete, and is `feedback.ts` truly the only tool turning agent strings into object keys?
4. Does **any** error message reachable over serve-http leak an absolute host path or cache layout?
5. Is **"zero outbound network calls in serve mode"** genuinely enforced for embeddings, reranker, AND OCR — including cache-miss paths? Or can a crafted query/PDF trigger a CDN fetch?
6. Is the **NFC class** now complete, or is there an identity/encoding surface (frontmatter keys, another comparator) still unfolded?
7. Any **write-path data-loss** beyond the rc.60/rc.61/rc.8 cases (case-insensitive FS, backlinks, non-mapping frontmatter, concurrent same-note writes)?
8. Any **test-theater** — a test (especially a security/invariant test) that would still pass if the code it guards were broken?
...
- Tag inline-extraction in `bases.ts` is ASCII-regex-scoped (`[A-Za-z]`) — narrower than `parser.ts` (`\p{L}`); note if you think it should match, but it's a known narrowing.
...
Plus: an executive summary (counts by severity), the **methodology you used** (for the diversity gate), the **commit SHA you graded**, and an explicit statement of any finding where you **re-verified and DISAGREE with the maintainer's rc.8/rc.9 verdict**.
...
Per-item re-verification against the graded commit is expected — stale findings (re-flagging something already fixed in rc.1→rc.9) and false positives both reduce signal; when uncertain, say so and show your repro attempt.
