...
| `--watch`              | off     | Watch the vault for `.md` add/change/unlink events (and `.pdf` if `--include-pdfs`). On change: invalidate the parsed-note cache for that file; if `--persistent-index` is also enabled, incrementally re-sync just that file's FTS5 chunks AND (since v3.8.0 for `.md`, v3.8.0 for `.pdf`) the embed-db rows when the embed-db file exists. **Since v3.9.0 with `--use-hnsw`:** the in-memory HNSW index is updated live so semantic search normally reflects edits within ~250ms (pre-3.9.0 needed serve restart). A failed per-file HNSW sync remains fail-soft for the current process, which may retain older graph state; **v3.12.0-rc.25 `@rc` preview:** suppresses persistence for that uncertain generation so a restart rebuilds rather than trusting a partial sidecar. **Startup in the same preview:** watched embedding capability is frozen for one server generation, changes observed before late sinks attach are replayed from final disk state, and a process-restart interlock prevents later admission after interrupted guarded startup. A stranded interlock requires the strict `clear-embeddings` recovery documented below. Editor saves are debounced via chokidar's `awaitWriteFinish`. `--exclude-glob` patterns are honored — edits to excluded paths don't fire. Off by default; opt in for long-running servers. |
...
| `--no-hnsw-persist`    | persist on | v2.16.0 — disable HNSW persistence sidecar. Default behavior: save `.hnsw.bin` + `.meta.json` next to `.embed.db`; reload it on the next serve when the embed-db signature matches. **v3.12.0-rc.25 `@rc` preview:** also suppresses saving automatically after an uncertain watcher HNSW sync, leaving restart signature validation/rebuild as the recovery path. |
...
**v2.16.0 — HNSW persistence:** when `--use-hnsw` is passed, the index is persisted to a sidecar `.hnsw.bin` + `.hnsw.meta.json` next to `.embed.db` after the first build. Subsequent serve starts load the sidecar when the embed-db signature matches; rebuild happens automatically on signature mismatch. Pass `--no-hnsw-persist` to disable. Since v3.12.0-rc.25, an uncertain watcher embedding-to-HNSW sync permanently suppresses persistence for that server generation, so restart validation/rebuild is used instead of saving a potentially partial graph.
