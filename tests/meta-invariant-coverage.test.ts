...
describe("META-invariant: exact structural census + NEGATIVE control coverage", () => {
  ...
  // migration adds 12 bounded candidates for 65 total, and m111, paired m112-m113, m114-m153,
  ...
  beforeAll(async () => {
    ...
    // NEGATIVE control: append the forty-four exact historical root call-node byte spans after
    ...
    expect(declarativeSealStart).toBeGreaterThan(m153BlockStart);
    ...
    // current topology has 35 frozen roots; wrapping multiline m154 reindents its
    // call-node bytes, invalidating the m154 identity, root, check, and matcher leaf
    // while leaving the shared matcher with exactly 34 bound owners.
    ...
    expect(sha256Text(discardedM154CallNode)).toBe("4ba7195a5c0555ca8cc5e68d63f83a04a026c806a120ff6db95e45663a228bca");
    const discardedProvenanceRoot = replaceExactly(
      matrixSource,
      `      ${legacyM154CallNode},`,
      ["      (", `        ${discardedM154CallNode},`, "        mcpbInputs.integrity", "      ),"].join("\n")
    );
    expect(preparedAudit.auditMatrix(discardedProvenanceRoot)).toEqual(
      expect.arrayContaining([
        expect.stringMatching(/release mutation hybrid current source must retain exact SHA-256/),
        expect.stringMatching(/release mutation hybrid current matrix slice must retain exact SHA-256/),
        expect.stringMatching(/release mutation hybrid legacy case release\.case\.m154 has no remaining root call/),
        expect.stringMatching(
          /release mutation hybrid shared primary matcher 84d0f85b5d5eb401cf80482bfec6ab0a2d4f60687d54d364f189bdc73052f680.*exact closed iterable\/runtime topology for 34 frozen root/
        )
      ])
    );
    ...
    // Registry loop and legacy m154, preserving the frozen global case order.
