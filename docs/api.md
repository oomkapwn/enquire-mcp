...
| `--watch`              | off     | Watch the vault for `.md` add/change/unlink events (and `.pdf` if `--include-pdfs`). On change: invalidate the parsed-note cache for that file; if `--persistent-index` is also enabled, incrementally re-sync just that file's FTS5 chunks AND (since v3.8.0 for `.md`, v3.8.0 for `.pdf`) the embed-db rows when the embed-db file exists. **v3.12.0-rc.26 `@rc` preview:** each add/change captures one physical generation, stages Markdown or PDF lexical+semantic work from one source read, revalidates after every await, retries one drift once, then commits FTS5 → EmbedDb → HNSW without yielding. Preparation failure keeps the previous generation. A sink-mutation failure quarantines semantic routing until restart; HNSW-only uncertainty falls back to EmbedDb and disables sidecar persistence. **Startup in the same preview:** watched embedding capability is frozen for one server generation, changes observed before late sinks attach are replayed from final disk state, and a process-restart interlock prevents later admission after interrupted guarded startup. A stranded interlock requires the strict `clear-embeddings` recovery documented below. Editor saves are debounced via chokidar's `awaitWriteFinish`. `--exclude-glob` patterns are honored — edits to excluded paths don't fire. Off by default; opt in for long-running servers. |
