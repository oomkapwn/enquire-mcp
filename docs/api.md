...
| `--no-hnsw-persist`    | persist on | v2.16.0 — disable HNSW persistence sidecar. Default behavior: save `.hnsw.bin` + `.meta.json` next to `.embed.db`; reload it on the next serve when the embed-db signature matches. |
...
| `doctor` (**v3.12.0-rc.15 `@rc` preview**; command since v2.11.0) | `--vault <path>` `[--tier <basic\|hybrid\|hybrid-live>]` `[--exclude-glob <pattern...>]` `[--read-paths <pattern...>]` `[--json]` | Source-state-preserving, tier-aware diagnostic (default `hybrid`). `basic` requires an accessible vault; `hybrid` additionally requires SQLite + transformers + HNSW, valid FTS5/embed DBs, and exact non-empty embedder/reranker cache artifacts; `hybrid-live` additionally requires PDF.js. OCR is advisory. SQLite is inspected from an in-memory byte snapshot, never through a source-path SQLite handle; active WAL/journal, unstable files, or files over 256 MB become `unverified`, not “missing.” JSON includes `tier`, `scope`, `limitations`, and per-check `required`; exit 0 means every required check is `ok`. READY is structural/runtime-only: it does not certify freshness, complete PDF coverage, watcher delivery, OCR packs, model artifact integrity/inference loadability, or at-rest privacy purge/index membership. Privacy flags affect the live vault counts only. Ordinary reads may update OS access-time metadata. |
| `setup` (v2.11.0) | `--vault <path>` `[--embedding-model <alias>]` `[--include-pdfs]` `[--skip-embeddings]` | Idempotently caches the selected embedder and builds/refreshes FTS5 + embed-db. **v3.12.0-rc.15 `@rc` preview:** for the generated exact hybrid tier, separately run `enquire-mcp install-model rerank-bge`; use `setup --include-pdfs` for hybrid-live. |
| `configure` (v3.11.6) | `--vault <path>` `[--client <name>]` `[--tier <basic\|hybrid\|hybrid-live>]` `[--name <name>]` `[--http]` | Print a ready-to-paste MCP client config for this vault — **non-destructive** (writes nothing). Emits the snippet + where it goes for Claude Code, Claude Desktop, Cursor, Windsurf, VS Code, Codex, or a remote HTTP client (omit `--client` for all). **v3.12.0-rc.15 `@rc` preview:** VS Code receives its official encoded review/install URI, Claude Code and Codex receive copy-and-run commands, and Marketplace/Registry-only clients are explicitly labeled copy-only; every route retains the generated config fallback. CLI output pins the physical Node/package entrypoint; `--tier basic` is a live scan, `hybrid` adds FTS5 + embeddings + reranker + HNSW, and `hybrid-live` adds PDFs + `--watch`; the preflight includes tier-matched setup, reranker caching and doctor verification. |
| `first-run` (**v3.12.0-rc.15 `@rc` preview**) | `--vault <path>` `[--client <name>]` `[--tier <basic\|hybrid\|hybrid-live>]` `[--name <name>]` `[--http]` `[--embedding-model <alias>]` `[--quantize-embeddings <mode>]` `[--exclude-glob <pattern...>]` `[--read-paths <pattern...>]` `[--apply]` | Package-coherent configure → prepare → doctor orchestration. **Preview is the default:** only non-destructive `configure` runs; the exact remaining commands and their effects are printed. `--apply` explicitly authorizes index/model-cache preparation, then hybrid tiers run idempotent `setup`, cache the verified `rerank-bge`, and run tier-aware `doctor`; basic skips all index/model work. Child commands use raw argument vectors against the same physical package entrypoint, stop at the first failure, preserve privacy/model/quantization choices, and print an idempotent resume command. |
| `eval` (v2.12.0) | `--vault <path>` `--queries <jsonl>` `[--k <n>]` `[--matrix]` `[--reranker]` `[--persistent-index]` `[--per-query]` `[--json]` `[--output <file>]` | Built-in retrieval-quality benchmark harness. Reads a JSONL file of queries with known-relevant doc paths (`{query, relevant: ["path1", ...], id?, category?}`), runs `obsidian_search` for each, computes **NDCG@K + Recall@K + MRR + Hit@1/Hit@K + AllRel@K** + per-query latency, and (v3.11.6) **groups the same metrics by `category`** with per-query `missed_paths`/`top_paths`. Pretty table by default; `--json` for machine output; `--matrix` runs a 2x2 (graph_boost ± reranker) comparison. **v3.12.0-rc.15 `@rc` preview:** `--output <file>` produces input for `enquire-mcp eval-compare`; degraded matrix rows are invalid, and comparison requires zero retrieval errors plus identical `k`, query count and canonical cohort fingerprint. `|Δ|≥0.01` is a material-effect CI threshold, not statistical significance. See [`docs/EVALUATION.md`](EVALUATION.md). |
| `eval-compare` (**v3.12.0-rc.15 `@rc` preview**) | `<baseline.json> <after.json>` `[--json]` | Packaged A/B comparator for `eval --output` results. Requires a clean, finite result pair with the same `k`, query count and canonical cohort fingerprint; preserves the source-checkout wrapper's matrix behavior by comparing the first row of each matrix output. Returns nonzero for malformed/mismatched/errored inputs or when any tracked aggregate metric regresses by the fixed `|Δ|≥0.01` material-effect threshold. This is a CI heuristic, not statistical significance. |
...
**v2.16.0 — HNSW persistence:** when `--use-hnsw` is passed, the index is persisted to a sidecar `.hnsw.bin` + `.hnsw.meta.json` next to `.embed.db` after the first build. Subsequent serve starts load the sidecar when the embed-db signature matches; rebuild happens automatically on signature mismatch. Pass `--no-hnsw-persist` to disable.
...
| `install-model` (v2.0+) | `[alias]` (default `multilingual`) | Explicitly download an embedding or reranker model into the transformers.js package-local cache. Embedders: `multilingual` (~120 MB, 50+ languages) and `bge` (~33 MB, English); rerankers include the verified default `rerank-bge` (~110 MB). Idempotent. **v3.12.0-rc.15 `@rc` preview:** all runtime commands (`serve`, `serve-http`, `query`, `eval`) fail closed on a cache miss; run tiered `doctor` from the same physical package copy to locate its exact cache. |
...
**Setup (one-time):**
```bash
enquire-mcp install-model multilingual          # ~120MB, cached in this package installation
