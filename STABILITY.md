...
`4.0.0-rc.3` is the current preview of the deliberate major boundary for the official MCP TypeScript SDK v2 and MCP protocol revision `2026-07-28`:
...
- HNSW persistence now writes an immutable `.hnsw.<nonce>.bin` generation and publishes a digest-bound `.hnsw.meta.json` pointer last. The exported `HnswPersistedMeta` v1 TypeScript declaration remains source-compatible, but v1 fixed-bin sidecars are not current authoring format and rebuild fail-soft.
