...
- **Retention and erasure.** `clear-cache --vault <path>` removes the parsed-note
  ...
  embedding database, WAL/SHM/rollback-journal files, HNSW sidecars, and the exact
  ...
  is dry-run by default and, with `--yes`, removes entries in Enquire's reserved
  artifact-name families (including feedback) for **other** legacy hash stems
  while keeping the named vault's stem. Here “current” and “other” are routing
  labels, not collision-proof vault identities: the default basename contains
  only the first 12 hex characters of SHA-1 over the vault path. An accidental
  cross-root collision can therefore share one default namespace; a deliberate
  same-account collision is inside the local-filesystem trust boundary. Preview
  `prune` output before `--yes`. Name recognition is not creator provenance: a
  same-account process that deliberately plants an exact reserved name is inside
  that same boundary. A noncanonical native
...
- **Plain-file publication and read boundary.** Parse-cache, feedback, and HNSW pointer/
  ...
  This prevents pre-planted deterministic-temp/final-symlink target writes and
  torn named files while the parent, path components, reserved entries, and
  staged-inode contents remain stable. Existing/custom parents remain
  operator-managed; active same-account entry/content tampering between
  validation, hashing, and rename/unlink is not an `openat`-style guarantee.
  Parent directories are not fsync'd, so power-loss durability is also not claimed.
  Parse-cache, feedback, and HNSW metadata content is read through a non-symlink
...
- Each cache generation is written through an exclusive unpredictable sibling, mode **`0600`** is applied before rename, and the published leaf is never chmod'd. A missing parent is requested with recursive mode **`0700`** mkdir, subject to a more-restrictive umask; an existing/custom parent is never path-chmod'd. The stable-entry/content and parent-fsync limits above apply.
- Cache content is rejected at load if its `root` field doesn't match the current vault realpath. This is a foreign-payload admission guard, not collision-proof file ownership: the legacy 12-hex SHA-1 default basename can route different roots to the same path and does not bind a later erasure operation to one root.
...
- Manual purge: `enquire-mcp clear-cache --vault <path>` deletes the admitted `.json` cache file plus recognized legacy/generated publisher leftovers.
...
- **One-generation live staging for ordinary updates** — each Markdown/PDF add or change captures device/inode/size plus nanosecond mtime/ctime, reads the source once, completes parsing/OCR/embedding work before a replacement-generation commit, and revalidates the path generation. One changed generation is retried once. A second drift keeps the previous live index state; the same condition rejects guarded activation.
...
- Cross-store crash atomicity: FTS5, EmbedDb and HNSW remain independent stores without one durable rollback journal. Source-scoped quarantine prevents a surviving process from querying a persisted sink-mutation mismatch for that path, but it does not undo already-written bytes or make a mid-commit process crash atomic; restart reconciliation/rebuild is the recovery boundary.
...
- **Best-effort 0600 chmod** on `<vault-hash>.embed.db` + WAL + SHM sidecar files after a successful admitted open where POSIX modes work; chmod failure does not replace a completed admission/bootstrap result. A missing parent is requested with recursive mode-`0700` mkdir, subject to a more-restrictive umask; an existing or explicitly selected parent is never path-chmod'd.
- **Ownership is admission, not a rebuild signal.** A populated database must identify the exact `vault_root` and embedding-store class before Enquire issues persistent PRAGMA, schema, metadata, or content changes. A foreign, missing-owner, malformed, or wrong-class store is refused; only an exact-root store may intentionally rebuild for an older supported schema or a model/dimension/quantization change. Before every disk-backed EmbedDb native open, the main/WAL/SHM/rollback-journal family is checked before dependency loading and again immediately before the constructor: every present leaf must be a singly linked regular file, and a missing main is accepted only with no sidecars. Node-`lstat`-recognized symlinks/junctions, special leaves, hardlinks, and orphan sidecars refuse. This assumes a stable parent and does not prove against a post-check entry swap or arbitrary Windows reparse tag. Refusal preserves the logical SQLite schema and committed SQL text/BLOB values, but an admitted SQLite/VFS open may still change locks, crash-recovery state, or WAL/SHM bookkeeping.
...
- **Configured persistence namespaces are exact.** Parse-cache, FTS5, embedding, feedback, and HNSW main/base paths end exactly in case-sensitive `.json`, `.fts5.db`, `.embed.db`, `.feedback.json`, and `.hnsw`, respectively; the parse-cache namespace excludes the reserved feedback and HNSW-meta JSON subclasses. Invalid spellings throw before the corresponding storage/erasure I/O, and only admitted database paths receive fail-soft diagnostic classification. This keeps mains, SQLite WAL/SHM/rollback journals, watcher guards, HNSW generations/pointers, and publisher leftovers disjoint while preserving arbitrary case in each filename stem. This is an intentional v4 prerelease compatibility boundary: arbitrary custom spellings accepted by v3 are not automatically migrated, and the exact erasers cannot safely reclaim an ambiguous pre-boundary path. Stop every enquire process, manually inspect/remove that legacy main and its sidecars, then configure a compliant replacement.
- **Caveat — the embed index stores recoverable content at rest.** The `text_preview` column of `.embed.db` stores the **raw leading text of each chunk** directly. Current compact HNSW pointers contain no paths or previews, but their native vector generations remain reversible-ish: with the same model loaded, an attacker with read access can run nearest-neighbor probes to recover note topics. Historical format-2 HNSW pointers also carried previews and remain covered by migration/erasure. Treat the `.embed.db` plus every `.hnsw.*` generation as having the same sensitivity as the `.fts5.db` (which already stores raw chunk content). If your threat model includes other local users on the same machine, do not use `--persistent-cache` / `--persistent-index` / build-embeddings. Enquire best-effort reasserts DB/WAL/SHM mode `0600`; HNSW generation/pointer mode is applied before publication. Missing parents are requested with recursive mode-`0700` mkdir subject to a more-restrictive umask; existing/custom parent modes remain operator-managed.
...
- **Manual purge and interrupted-startup recovery.** `enquire-mcp clear-embeddings --vault <path>` removes the `.embed.db`, `.embed.db-wal`, `.embed.db-shm`, and `.embed.db-journal` files, the HNSW stable `.hnsw.meta.json` pointer, immutable `.hnsw.<nonce>.bin` generations, legacy fixed `.hnsw.bin`, recognized publisher temp/stage leftovers, and the token-only watcher startup interlock. Before deleting the first artifact it read-only preflights the guard and every reserved artifact shape. Recognized file/symlink leaves are unlinked without following their targets; a staging symlink, special/malformed object, ambiguous spelling, unsafe guard, or unexpected guard entry refuses recovery for manual inspection. Recoverable artifacts are then removed before the interlock, whose exact shape is revalidated and removed last. Automatic writer/search recovery guidance first performs a separate read-only ownership check: a missing database is the expected stranded-guard case, while a present database must prove the exact supported EmbedDb class and vault root; foreign, malformed, future, unreadable, or close-failing stores receive only a generic path-free error, not clear/rebuild instructions. Any permission/type/race error is loud, and cleanup never recursively deletes unexpected content. Stop every enquire process first, use a custom path ending exactly in `.embed.db`, repeat that matching `--embed-file`, and rebuild with the same model, quantization, late-chunk, privacy and PDF settings. The stable-entry/content and active same-account tampering boundary above applies to deletion too.
...
- A missing parent is requested with recursive mode **`0700`** mkdir, subject to a more-restrictive umask; an existing or explicitly selected parent is never path-chmod'd and remains operator-managed.
...
- Ownership is admission, not a rebuild signal: a populated database must identify the exact `vault_root` and FTS store class before Enquire issues persistent PRAGMA, schema, metadata, or content changes. Production discovery returns only `missing`, exactly schema-`empty`, exact-root `owned`, or generic `refused`; defaults are safe only for the first two, and a writer independently repeats admission. A foreign, missing-owner, malformed, or wrong-class store is refused; only an exact-root store may intentionally rebuild for an older supported schema or explicit tokenizer change. Before every disk-backed FTS native open, the main/WAL/SHM/rollback-journal family is checked before dependency loading and again immediately before the constructor: every present leaf must be a singly linked regular file, and a missing main is accepted only with no sidecars. Node-`lstat`-recognized symlinks/junctions, special leaves, hardlinks, and orphan sidecars refuse. This assumes a stable parent and does not prove against a post-check entry swap or arbitrary Windows reparse tag. Runtime and stored tokenizer values must be exactly `unicode61` or `trigram`; unknown values are never coerced to the default. Refusal preserves the logical SQLite schema and committed cells, but does not promise container-byte, lock, recovery, or WAL/SHM identity across an admitted SQLite/VFS open.
- Manual purge: `enquire-mcp clear-index --vault <path>` preflights and removes the `.fts5.db`, `.fts5.db-wal`, `.fts5.db-shm`, and `.fts5.db-journal` files; unsafe shapes or non-ENOENT failures are loud.
...
When `--feedback-weight > 0`, the `obsidian_mark_useful` tool records which recalled notes helped a query into a routing-key-scoped JSON sidecar at `<vault-hash>.feedback.json` (same cache dir as the parse cache / FTS5 index). For non-colliding roots this behaves per-vault; the legacy first-12-hex SHA-1 stem is not collision-proof root identity.
...
- **Publication posture.** Each sidecar generation uses an exclusive unpredictable mode-`0600` sibling, file fsync, then rename; the published leaf is never chmod'd. A missing parent is requested with recursive mode-`0700` mkdir subject to a more-restrictive umask; an existing/custom parent is never path-chmod'd. This is atomic leaf replacement under the stable-entry/content boundary above, not a parent-fsync/power-loss guarantee.
- **Bounded.** At most `MAX_FEEDBACK_ENTRIES` (100,000) distinct notes are tracked; at the cap, existing entries still update but new paths are ignored. A serialized generation over `MAX_FEEDBACK_FILE_BYTES` (64 MiB) is refused before temp reservation/publication, and reopen enforces the same limit on the held descriptor before parsing. This bounds both path-cardinality growth and persisted read/write size from a misbehaving `serve-http` client.
- **Right-to-erasure.** `enquire-mcp prune` selects `<hash>.feedback.json`, legacy `.tmp`, recognized generated temp/stage leftovers, and family-valid SQLite WAL/SHM/rollback journals under routing stems other than the one kept, alongside the other reserved cross-stem erasable families — pinned by the erasure-completeness invariant (`tests/erasure-invariant.test.ts`). For non-colliding roots this removes a decommissioned vault's recognized family; a shared SHA1-12 stem cannot distinguish roots and remains the AH-9e migration residual. It preflights every selected artifact before deletion and fails loud on unsafe shapes or non-ENOENT inspection/deletion errors. Watcher startup interlocks are deliberately excluded from cross-vault GC because removing another vault's interlock could weaken a live/crash recovery boundary; only exact-vault `clear-embeddings` recovery may remove one. Feedback is **preserved across `clear-cache`** (it is user-generated signal, not regenerable cache); preview and then run `prune --yes` to remove it. Exact reserved-name impersonation and active entry/content tampering remain outside the guarantee above.
- **Erasing the ACTIVE vault's marks.** `prune` above reaches only OTHER routing stems, and `clear-cache` deliberately excludes this subclass because the marks are user-recorded state rather than derived data. `enquire-mcp clear-feedback --vault <path>` is the command that removes the current vault's sidecar and any generated sibling, under the exclusive family lease and the shared erasure-receipt rule (only `ENOENT` is idempotent success; a removal is believed only once the entry is re-statted absent). A `serve` running with `--feedback-weight > 0` republishes the sidecar on its next recorded mark, so stop it first when the intent is permanent erasure.
...
- **Body bombs.** Per-request body size cap derived from `--max-file-bytes` as `max(4 MiB, max-file-bytes × 6 + 64 KiB)` (~30.06 MiB at the default 5 MiB file cap: JSON `\u00xx` expansion plus envelope headroom), enforced as we accumulate chunks. Overflow returns `413 Request body too large` before JSON.parse; malformed bodies still get `400 Parse error`.
