...
- Tests: `tests/*.test.ts` (Vitest, 966+ tests)
...
- OIA: `npm run check:oia` (state-driven drift scan — 9 checks)
...
├── search.ts           — RRF fusion + reranking orchestration
...
scripts/
├── oia-walk.mjs                       — state-driven drift scan (9 checks)
