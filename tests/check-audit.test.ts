...
describe("check-audit scoped gate (rc.50)", () => {
  it("flags an un-allowlisted advisory at/above threshold (NEGATIVE control — not vacuous)", () => {
    const offenders = offendingAdvisories(sample, { minSeverity: "moderate", allowlist: ALLOWLIST });
    ...
    expect(ids).not.toContain("GHSA-h67p-54hq-rp68"); // the documented js-yaml advisory is allowed
    expect(ids).not.toContain("GHSA-dddd-eeee-ffff"); // below threshold (low < moderate) ignored
  });

  it("passes clean once only the allowlisted advisory remains (POSITIVE)", () => {
    const onlyAllowlisted = { vulnerabilities: { "js-yaml": sample.vulnerabilities["js-yaml"] } };
    expect(offendingAdvisories(onlyAllowlisted, { minSeverity: "moderate", allowlist: ALLOWLIST })).toEqual([]);
  });

  it("the production js-yaml advisory is the only allowlisted entry (drift guard)", () => {
    // If a future advisory gets allowlisted, this assertion forces a conscious update +
    // keeps the allowlist from silently growing.
    expect(Object.keys(ALLOWLIST)).toEqual(["GHSA-h67p-54hq-rp68"]);
  });
});
