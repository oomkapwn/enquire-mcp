...
**v4 protocol boundary.** `4.0.0-rc.4` uses the exact official MCP SDK v2 packages to serve modern `2026-07-28` and supported legacy clients from this same registered surface. Modern HTTP requests are strict and per-request; `--stateful` preserves the legacy initialize/session/SSE/DELETE path. A malformed or unsupported modern claim never downgrades into a legacy session.
...
**v2.16.0 — HNSW persistence (format 3 in v4):** when `--use-hnsw` is passed, the index is written as an immutable `.hnsw.<nonce>.bin` generation next to `.embed.db`; `.hnsw.meta.json` is published last as the stable basename + SHA-256 pointer. The compact pointer is capped at 64 KiB and contains no row paths or previews; the native generation is capped at 1 GiB and is admitted against one atomic DB-owned label manifest before C++ load. Writer peak memory remains proportional to the in-memory graph. An oversized pointer/generation is not committed; precommit orphan cleanup is best-effort, and strict generated residue remains covered by explicit clear/prune. Subsequent serve starts load only a digest-matching, compact, tombstone-free generation whose EmbedDb receipt and exact live labels match; malformed, legacy-v1/v2, missing, mixed, or stale pairs rebuild automatically. An uncertain watcher embedding-to-HNSW sync quarantines the live HNSW route, falls back only when the EmbedDb generation itself passed complete admission, and suppresses persistence for that server generation.
