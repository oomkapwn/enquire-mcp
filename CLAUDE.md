...
- **v3.8.0-rc.9 shipped (current)**: round-7 external audit response (3 fixes). W-FLAKE-2 50ms chokidar warmup missing from R-7 embed tests (sibling of rc.7 #36 incomplete sweep); R-10 HNSW k multiplier 4× → 6× limit + TSDoc documentation of residual under-return; N-new qs 6.15.1 → 6.15.2 (GHSA-q8mj-m7cp-5q26 DoS). **842 tests** (unchanged).
- **v3.8.0 backlog status**: All round-24 findings closed. Remaining v3.8.0 items: serve-http parity (P1-3), OCR concurrency/timeout, search underfill (P2-7), FTS5/embedding chunking parity (P2-8), stateful session race (P2-10), HTTP server close cleanup (P2-11), doctor privacy filters (P2-12), npm package broken links (P2-18), canonical CLI docs (P2-20), FTS gating wording (P3-21), heading tilde fences (P3-25), HNSW metadata validation (P3-27), setup-snippet mkdir (P3-29) + other v3.8.0 backlog originals. External audit before promotion to v3.8.0 stable.
