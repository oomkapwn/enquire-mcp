...
[![tests](https://img.shields.io/badge/tests-2270%20contracts-brightgreen.svg)](#️-доверие)
...
**[⚡ Установка за 30 секунд](#-быстрый-старт) · [🏆 Почему №1](#why-number-one) · [🧠 Сценарии использования](#-сценарии-использования) · [📊 Бенчмарки](./docs/benchmarks.md) · [📖 Справочник API](https://oomkapwn.github.io/enquire-mcp/api/)**
...
**47 инструментов · 19 MCP-промптов · 2270+ модульных тестов · 50+ языков · стабильная ветка v3.11.x · с гарантиями semver · MIT · подтверждённая сборка в npm (SLSA L2).**
...
| **Продакшен-доверие** | ✅ Read-only по умолчанию, privacy-фильтры, авторизованный HTTP, semver-контракты, 2270 тестов, 13 релизных гейтов и SLSA L2 provenance |
...
- *«Каким было моё решение по PostgreSQL против MongoDB? Сошлись на дневниковую заметку.»* — при равных RRF-оценках объединённых кандидатов число входящих wikilink-ссылок внутри набора кандидатов разрешает ничью
...
`obsidian_search` автоматически определяет доступные сигналы и плавно деградирует. Усиление по wikilink-графу только разрешает ничьи между равными RRF-оценками по числу входящих ссылок внутри набора кандидатов. Опциональный кросс-энкодер-реранкинг переоценивает top-N, давая +15.5 NDCG@10 по замерам. Каждый результат возвращает `per_signal: { bm25, tfidf, embeddings }`, так что вы видите, ПОЧЕМУ он так ранжирован.
...
## 🛠️ Все 47 инструментов

Всего 47 инструментов: 35 всегда включённых на чтение (включая зонтичный `obsidian_search`) + 4 опциональных на чтение + 7 управляемых на запись + 1 инструмент обратной связи с замкнутым контуром. Полный справочник: **[docs/api.md](./docs/api.md)**.
...
| **2270 модульных тестов · 13 обязательных для релиза CI-проверок · все 13 сейчас защищают ветку** | Текущая проверенная релизная позиция; операционная детализация закреплена ниже. |
...
npm test       # полный набор (2270 тестов)
