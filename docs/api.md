...
| `install-model` (v2.0+) | `[alias]` (default `multilingual`) | Explicitly download an embedding or reranker model into the transformers.js package-local cache. Embedders: `multilingual` (~120 MB, 50+ languages) and `bge` (~33 MB, English); rerankers include the verified default `rerank-bge` (~110 MB). Idempotent. **v3.12.0-rc.14 `@rc` preview:** all runtime commands (`serve`, `serve-http`, `query`, `eval`) fail closed on a cache miss; run tiered `doctor` from the same physical package copy to locate its exact cache. |
