...
This source guide also documents the `v3.12.0-rc.17` preview: tier-aware source-preserving doctor, preview-first `first-run`, configuration-aware MCP instructions, and verified client-specific install actions. Until that release reaches `@latest`, install that exact prerelease globally, then keep preparation and generated runtime on this one installation:

```bash
npm install -g @oomkapwn/enquire-mcp@3.12.0-rc.17
enquire-mcp --version
```

Expected output: the newest `@latest` version for the stable path, or exactly `3.12.0-rc.17` after selecting the preview. The [CHANGELOG](../CHANGELOG.md) identifies the exact contents of each channel.
