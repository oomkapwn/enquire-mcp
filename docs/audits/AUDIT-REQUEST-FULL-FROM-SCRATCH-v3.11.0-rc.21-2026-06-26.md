# External Audit Request — enquire-mcp v3.11.0-rc.21 (FULL, from scratch, obligation-gated, comparison-ready)

**Status:** OPEN — **promotion-gate audit for v3.11.0 → `@latest`**.
**Date issued:** 2026-06-26
**Supersedes:** `AUDIT-REQUEST-FULL-FROM-SCRATCH-v3.11.0-rc.17-2026-06-24.md` (this brief is a from-the-ground-up redesign — see §13 "Why this brief is different" — informed by what 4 LLM auditors of rc.17 did well and badly).
...
git checkout v3.11.0-rc.21 && git rev-parse HEAD   # expect e016266… — record the FULL SHA in your report
npm view @oomkapwn/enquire-mcp@rc version            # must read 3.11.0-rc.21
```

> The authoritative target is the commit **tagged `v3.11.0-rc.21`** (short SHA `e016266`). Grade THAT commit, not whatever `main` currently points at (this brief itself lands on `main` as a later doc-only commit). Cite the SHA in every finding. `@latest` is **3.10.1** (stable); the v3.11.0 line is on `@rc` pending this audit + the **≥2-independent-auditor** promotion gate.
