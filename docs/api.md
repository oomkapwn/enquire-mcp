...
**v4 protocol boundary.** `4.0.0-rc.2` uses the exact official MCP SDK v2 packages to serve modern `2026-07-28` and supported legacy clients from this same registered surface. Modern HTTP requests are strict and per-request; `--stateful` preserves the legacy initialize/session/SSE/DELETE path. A malformed or unsupported modern claim never downgrades into a legacy session.
