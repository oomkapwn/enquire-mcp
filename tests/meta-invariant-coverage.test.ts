...
describe("META-invariant: exact structural census + NEGATIVE control coverage", () => {
  ...
  // migration adds 12 bounded candidates for 65 total, and m111, paired m112-m113, m114-m150,
  ...
  beforeAll(async () => {
    ...
    // NEGATIVE control: append the forty-one exact historical root call-node byte spans after
    ...
    expect(declarativeSealStart).toBeGreaterThan(m150BlockStart);
    ...
    // current topology has 37 frozen roots; wrapping multiline m154 reindents its
    // call-node bytes, invalidating the m154 identity, root, check, and matcher leaf
    // while leaving the shared matcher with exactly 36 bound owners.
    ...
    expect(preparedAudit.auditMatrix(discardedProvenanceRoot)).toEqual(
      expect.arrayContaining([
        ...
        expect.stringMatching(
          /release mutation hybrid shared primary matcher 84d0f85b5d5eb401cf80482bfec6ab0a2d4f60687d54d364f189bdc73052f680.*exact closed iterable\/runtime topology for 36 frozen root/
        )
      ])
    );
    ...
    // Registry loop and legacy m151, preserving the frozen global case order.
