...
**47 أداة · 19 موجِّه MCP · 2270+ اختبار وحدة · 50+ لغة · إصدار مستقر v3.11.x · مُقيَّد بالـ semver · MIT · إثبات بناء npm (SLSA L2).**
...
| **ثقة الإنتاج** | ✅ قراءة فقط افتراضياً ومرشحات خصوصية وHTTP موثّق وعقود semver و2270 اختباراً و13 بوابة إصدار ومصدر SLSA L2 |
...
يوفّر [GitHub Release `v4.0.0-rc.7`](https://github.com/oomkapwn/enquire-mcp/releases/tag/v4.0.0-rc.7) ملف `enquire-mcp-basic-4.0.0-rc.7.mcpb` مع checksum والجرد وSBOM والإشعارات وإثبات المصدر. تتضمن الحزمة JavaScript الخاص بالخادم والاعتماديات العادية، وعلى مضيف MCPB المتوافق توفير Node.js 22.13 أو أحدث.
...
- *"ما قراري بشأن PostgreSQL مقابل MongoDB؟ استشهِد بالملاحظة اليومية."* — عند تساوي درجات RRF للمرشحين المدمجين، يكسر عدد روابط Wikilink الواردة ضمن مجموعة المرشحين التعادل
...
**2 — قاعدة معرفة شخصية / دماغ ثانٍ.** يُظهر الاسترجاع الهجين الملاحظة الصحيحة لـ*أي* صياغة، في أيٍّ من 50+ لغة. اسأل بالإنجليزية عن مُدخل يوميات بالروسية من قبل عامين، واحصل على النتيجة الصحيحة. لا يفعل Wikilink graph-boost سوى كسر التعادل بين درجات RRF المتساوية باستخدام عدد الروابط الواردة ضمن مجموعة المرشحين. ويُظهر GraphRAG-light المجتمعات الموضوعية — لتكتشف روابط نسيت أنك صنعتها. وتندمج ملفات PDF في البحث مع استشهادات `[page: N]`، فتصير الأوراق البحثية ونصوص الاجتماعات ذاكرةً من الدرجة الأولى.
...
يكتشف `obsidian_search` الإشارات المتاحة تلقائياً ويتراجع بلُطف. لا يفعل Wikilink graph-boost سوى كسر التعادل بين درجات RRF المتساوية باستخدام عدد الروابط الواردة ضمن مجموعة المرشحين. وتُعيد إعادة الترتيب الاختيارية بمُرمِّز متقاطع تسجيل top-N للحصول على ‎+15.5 NDCG@10 مقاسة. تُعيد كل نتيجة `per_signal: { bm25, tfidf, embeddings }`، فترى *لماذا* احتلّت ترتيبها.
...
47 أداة إجمالاً: 35 قراءة دائمة (بما في ذلك المدخل الجامع `obsidian_search`) + 4 اختيارية + 7 كتابات محكومة + 1 تغذية راجعة بحلقة مغلقة. المرجع الكامل في **[docs/api.md](./docs/api.md)**.
...
| **2270 اختبار وحدة · 13 فحص CI مطلوباً للإصدار · جميع الفحوص الـ13 محمية حالياً على الفرع** | وضع إصدار متحقق منه؛ والتفاصيل التشغيلية مثبتة أدناه. |
...
npm test       # full suite (2270 tests)
