...
const RELEASE_MUTATION_NPM_PROBLEM =
...
function releaseMutationInventoryProblems(source: string): string[] {
  ...
  let directNpmContractDeclarations = 0;
  let otherNpmContractBindings = 0;
  ...
  const releaseOracleAdapterIdentity = (
    ...
    if (
      property.name.text === RELEASE_MUTATION_NPM_ADAPTER_PROPERTY &&
      ts.isIdentifier(property.initializer) &&
      property.initializer.text === "npmProvenanceContractProblems"
    ) {
      return "npm.contract";
    }
    if (
  ...
  const closedReleaseOracleAdapterObject = (value: ts.Expression | undefined): value is ts.ObjectLiteralExpression => {
    ...
    const identities = value.properties.map(releaseOracleAdapterIdentity);
    return identities.every((identity) => identity !== null) && new Set(identities).size === identities.length;
  };
  const exactReleaseOracleAdapterObject = (
    ...
    if (
      ...
      !closedReleaseOracleAdapterObject(value) ||
      value.properties.length !== expectedIdentities.length
    ) {
      return false;
    }
    const identities = new Set(value.properties.map(releaseOracleAdapterIdentity));
    return expectedIdentities.every((identity) => identities.has(identity));
  };
  const isExactReleaseOracleAdapterInitializer = (node: ts.Identifier): boolean => {
    const property = node.parent;
    if (!ts.isPropertyAssignment(property) || property.initializer !== node) return false;
    const object = property.parent;
    if (!closedReleaseOracleAdapterObject(object)) return false;
  ...
  const visitRuntimeBindings = (node: ts.Node): void => {
    ...
    if (
      ...
      node.initializer !== undefined &&
      ts.isIdentifier(node.initializer) &&
      node.initializer.text === "npmProvenanceContractProblems"
    ) {
  ...
  if (directNpmContractDeclarations !== 1 || otherNpmContractBindings !== 0) {
    problems.push(
      `release mutation npm contract adapter must have one top-level function declaration and no other runtime bindings; found direct ${directNpmContractDeclarations}, other ${otherNpmContractBindings}`
    );
  }
  if (npmContractAliasInitializers !== 0) {
    problems.push(
      `release mutation npm contract adapter must not have alias initializers; found ${npmContractAliasInitializers}`
    );
  }
  if (npmContractWrites !== 0) {
    problems.push(
      `release mutation npm contract adapter binding must never be reassigned; found ${npmContractWrites} write(s)`
    );
  }
    ...
    if (
      ...
      if (!directCall || !exactReceiver || !exactProperty) {
        ...
        if (
          ...
          const exactReleaseWorkflowFixtureValue =
            value !== undefined &&
            ts.isPropertyAccessExpression(value) &&
            value.questionDotToken === undefined &&
            ts.isIdentifier(value.expression) &&
            value.expression.text === "mcpbInputs" &&
            value.name.text === "release";
          if (
            parent.arguments.length !== 2 ||
            id === undefined ||
            !ts.isStringLiteral(id) ||
            value === undefined ||
            (!ts.isIdentifier(value) && !ts.isStringLiteral(value) && !exactReleaseWorkflowFixtureValue) ||
            handle === null
          ) {
            const position = sourceFile.getLineAndCharacterOfPosition(start);
            problems.push(
              `release mutation declarative registerSource requires one top-level const handle, literal id and passive identifier/string source value or the exact mcpbInputs.release binding at ${position.line + 1}:${position.character + 1}`
            );
          } else {
            if (
            ...
            if (
              ...
              (handle !== "registryPublishStepSource" || !ts.isIdentifier(value) || value.text !== "registryRun")
            ) {
              const position = sourceFile.getLineAndCharacterOfPosition(start);
              problems.push(
                `release mutation declarative workflow.registry-publish-step source requires the exact registryPublishStepSource/registryRun binding at ${position.line + 1}:${position.character + 1}`
              );
            }
            if (
              id.text === "fixture.release-workflow" &&
              (handle !== "releaseWorkflowFixtureSource" || !exactReleaseWorkflowFixtureValue)
            ) {
              const position = sourceFile.getLineAndCharacterOfPosition(start);
              problems.push(
                `release mutation declarative fixture.release-workflow source requires the exact releaseWorkflowFixtureSource/mcpbInputs.release binding at ${position.line + 1}:${position.character + 1}`
              );
            }
            if (
            ...
            if (
              id.text === "fragment.npm-provenance-evaluator-command" &&
              (handle !== "npmProvenanceEvaluatorCommandSource" ||
                !ts.isIdentifier(value) ||
                value.text !== "NPM_PROVENANCE_EVALUATOR_COMMAND")
            ) {
              const position = sourceFile.getLineAndCharacterOfPosition(start);
              problems.push(
                `release mutation declarative fragment.npm-provenance-evaluator-command source requires the exact npmProvenanceEvaluatorCommandSource/NPM_PROVENANCE_EVALUATOR_COMMAND binding at ${position.line + 1}:${position.character + 1}`
              );
            }
            if (
              id.text !== "script.release-integrity" &&
              ...
              const position = sourceFile.getLineAndCharacterOfPosition(start);
              problems.push(
                `release mutation declarative source ${id.text} is outside the closed source inventory at ${position.line + 1}:${position.character + 1}`
              );
            }
        ...
        const expectedInvocationProperties =
          ...
          invocationKind === "registry.evaluator"
            ...
            : invocationKind === "fixture.throw"
              ...
              : invocationKind === "registry.step.run" || invocationKind === "npm.contract.release"
  ...
  const exactExecutionAdapterArguments = (call: ts.CallExpression, offset: number): boolean => {
    ...
    return (
      ...
      exactReleaseOracleAdapterObject(
  ...
  const requiredAdapterDescription =
    requiredAdapterNames.length === 0
      ? "zero adapter arguments without closed production invocations"
      : `one exact literal ${requiredAdapterNames.join("/")} adapter object`;
...
for (( page_attempt=1; page_attempt<=2; page_attempt++ )); do
  ...
  it("keeps release.yml wired to the shared evaluator and an exact mirrored inventory", () => {
    ...
    expect(releaseMutationInventoryProblems(extraProjectMutation)).toContain(
      "release mutation hybrid inventory expected 538 first / 22 all, found 539 first / 22 all (legacy 449/19; declarative 90/3; cases 90)"
    );
    ...
    expect(releaseMutationInventoryProblems(projectModeDrift)).toContain(
      "release mutation hybrid inventory expected 538 first / 22 all, found 537 first / 23 all (legacy 447/20; declarative 90/3; cases 90)"
    );
    ...
    const npmProvenanceEvaluatorCommandSourceToken = [
      '"fragment.npm-provenance-evaluator-command",',
      "NPM_PROVENANCE_EVALUATOR_COMMAND"
    ].join("\n      ");
    const npmProvenanceEvaluatorCommandSourceOffset = declarativeBatchOffset(npmProvenanceEvaluatorCommandSourceToken);
    const transplantedNpmProvenanceEvaluatorCommandSource = [
      hybridDeclarativeMutation.slice(0, npmProvenanceEvaluatorCommandSourceOffset),
      ['"fragment.npm-provenance-evaluator-command",', "releaseIntegrityText"].join("\n      "),
      hybridDeclarativeMutation.slice(
        npmProvenanceEvaluatorCommandSourceOffset + npmProvenanceEvaluatorCommandSourceToken.length
      )
    ].join("");
    expect(releaseMutationInventoryProblems(transplantedNpmProvenanceEvaluatorCommandSource)).toContainEqual(
      expect.stringMatching(/fragment\.npm-provenance-evaluator-command source requires the exact/)
    );
    ...
    for (const nonPassiveM138Replacement of [
      ...
      const nonPassiveM138Template = replaceExactly(
        ...
        m138ReplacementTemplateToken,
        nonPassiveM138Replacement
      );
      expect(releaseMutationInventoryProblems(nonPassiveM138Template)).toContainEqual(
        expect.stringMatching(/descriptor replacement must be one passive string value or mutation handle/)
      );
    }
    ...
    const m144ReplacementHandleToken = "      replacement: releaseMutationM145,";
    const m144ReplacementHandleOffset = declarativeBatchOffset(m144ReplacementHandleToken);
    const sourceBackedM144Replacement = [
      hybridDeclarativeMutation.slice(0, m144ReplacementHandleOffset),
      "      replacement: npmProvenanceEvaluatorCommandSource,",
      hybridDeclarativeMutation.slice(m144ReplacementHandleOffset + m144ReplacementHandleToken.length)
    ].join("");
    expect(releaseMutationInventoryProblems(sourceBackedM144Replacement)).toContainEqual(
      expect.stringMatching(/descriptor replacement must be one passive string value or mutation handle/)
    );
    ...
    const npmReleaseInvocationKindToken = 'kind: "npm.contract.release"';
    const npmReleaseInvocationKindOffset = declarativeBatchOffset(npmReleaseInvocationKindToken);
    const reversedNpmReleaseInvocation = [
      hybridDeclarativeMutation.slice(0, npmReleaseInvocationKindOffset),
      'kind: "npm.contract.integrity"',
      hybridDeclarativeMutation.slice(npmReleaseInvocationKindOffset + npmReleaseInvocationKindToken.length)
    ].join("");
    ...
    const npmProblemExpectationOffset = declarativeBatchOffset(npmProblemExpectationToken);
    const wrongNpmProblemIdentity = [
      hybridDeclarativeMutation.slice(0, npmProblemExpectationOffset),
      [
        'id: "release.expectation.m112.primary",',
        '            kind: "problem",',
        '            problem: "fixture.mutant-threw"'
      ].join("\n"),
      hybridDeclarativeMutation.slice(npmProblemExpectationOffset + npmProblemExpectationToken.length)
    ].join("");
    expect(releaseMutationInventoryProblems(wrongNpmProblemIdentity)).toContainEqual(
      expect.stringMatching(/requires one exact problem identity/)
    );
    ...
    const missingReleaseOracleAdapters = [
    ...
    const aliasedNpmContractAdapter = [
      hybridDeclarativeMutation.slice(0, matrixBodyOffset),
      "\n    const npmContractAlias = npmProvenanceContractProblems;",
      hybridDeclarativeMutation.slice(matrixBodyOffset)
    ].join("");
    expect(releaseMutationInventoryProblems(aliasedNpmContractAdapter)).toContain(
      "release mutation npm contract adapter must not have alias initializers; found 1"
    );
    const shadowedNpmContractAdapter = [
      hybridDeclarativeMutation.slice(0, matrixBodyOffset),
      "\n    const npmProvenanceContractProblems = (_release: string, _integrity: string): string[] => [];",
      hybridDeclarativeMutation.slice(matrixBodyOffset)
    ].join("");
    expect(releaseMutationInventoryProblems(shadowedNpmContractAdapter)).toContainEqual(
      expect.stringMatching(
        /npm contract adapter must have one top-level function declaration and no other runtime bindings/
      )
    );
    const reassignedNpmContractAdapter = [
      hybridDeclarativeMutation.slice(0, matrixBodyOffset),
      "\n    npmProvenanceContractProblems = (_release: string, _integrity: string): string[] => [];",
      hybridDeclarativeMutation.slice(matrixBodyOffset)
    ].join("");
    ...
    expect(releaseMutationInventoryProblems(legacyFreeMatrix)).toContain(
      "release mutation final closed graph expected 560 unique descriptors / 536 cases and roots / 541 expectations / 24 dependency-only, found 93 descriptors / 90 cases / 90 roots / 90 expectations / 3 dependency-only"
    );
    ...
    expect(nestedStraightLineProblems).toContain(
      "release mutation hybrid inventory expected 538 first / 22 all, found 540 first / 22 all (legacy 450/19; declarative 90/3; cases 90)"
    );
    ...
    const createNpmContractPlan = (): ReleaseMutationPlan => {
      const plan = new ReleaseMutationPlan({
        total: 2,
        first: 2,
        all: 0,
      ...
      const releaseSource = plan.registerSource("fixture.npm-release", "release-clean");
      const integritySource = plan.registerSource("fixture.npm-integrity", "integrity-clean");
      const releaseRoot = registerFixtureMutation(plan, "mutation.npm-release", {
        mode: "first",
      ...
      const integrityRoot = registerFixtureMutation(plan, "mutation.npm-integrity", {
        mode: "first",
        source: integritySource,
        needle: "clean",
        replacement: "mutant",
        ...
        checks: [
          {
            invoke: {
              kind: "npm.contract.release",
              baseline: releaseSource,
              mutant: releaseRoot,
              integrity: integritySource
            },
            expectation: {
              id: "expectation.npm-release",
              kind: "problem",
    ...
    const npmPositivePlan = createNpmContractPlan();
    expect(npmPositivePlan.seal()).toEqual([]);
    const npmPositiveCalls: Array<readonly [release: string, integrity: string]> = [];
    ...
    expect(npmPositiveCalls).toEqual([
      ["release-clean", "integrity-clean"],
      ["release-mutant", "integrity-clean"],
      ["release-clean", "integrity-clean"],
      ["release-clean", "integrity-mutant"]
    ]);
    expect(npmPositivePlan.phase).toBe("executed");
    expect(npmPositivePlan.caseExecutions).toBe(2);
    expect(npmPositivePlan.expectationExecutions).toBe(2);

    const npmMissingAdapterPlan = createNpmContractPlan();
    expect(npmMissingAdapterPlan.seal()).toEqual([]);
    expect(() => npmMissingAdapterPlan.execute()).toThrow(
      "npm.contract cases require exact release oracle adapters at execute"
    );
    expect(npmMissingAdapterPlan.phase).toBe("failed");
    expect(npmMissingAdapterPlan.caseExecutions).toBe(0);
    expect(npmMissingAdapterPlan.expectationExecutions).toBe(0);

    const npmDirtyBaselinePlan = createNpmContractPlan();
    expect(npmDirtyBaselinePlan.seal()).toEqual([]);
    const npmDirtyBaselineCalls: Array<readonly [release: string, integrity: string]> = [];
    expect(() =>
      npmDirtyBaselinePlan.execute({
        npmContractProblems: (release, integrity) => {
          npmDirtyBaselineCalls.push([release, integrity]);
          return [RELEASE_MUTATION_NPM_PROBLEM];
        }
      })
    ).toThrow(
      "release mutation case case.npm-release expectation expectation.npm-release " +
        "found a problem in the clean baseline"
    );
    expect(npmDirtyBaselineCalls).toEqual([
      ["release-clean", "integrity-clean"],
      ["release-mutant", "integrity-clean"]
    ]);
    expect(npmDirtyBaselinePlan.phase).toBe("failed");
    ...
    releaseMutationPlan.registerCase({
      id: "release.case.m112",
      root: releaseMutationM112,
      checks: [
        {
          invoke: {
            kind: "npm.contract.release",
    ...
    const releaseMutationM113 = releaseMutationPlan.registerMutation("release.m113", {
      mode: "first",
      source: releaseIntegritySource,
      needle: 'eventName: "push"',
      replacement: 'eventName: "workflow_dispatch"',
      expectedOccurrences: 1,
      witness: {
        kind: "token",
        anchor: 'eventName: "push"',
        before: 1,
        after: 0
      }
    });
    releaseMutationPlan.registerCase({
      id: "release.case.m113",
      root: releaseMutationM113,
      checks: [
        {
          invoke: {
            kind: "npm.contract.integrity",
          ...
          expectation: {
            id: "release.expectation.m113.primary",
            kind: "problem",
            problem:
              "npm provenance must bind the tag-push context before the sole publish and verify two exact attestations without credentials"
          }
    ...
    const releaseMutationM117 = releaseMutationPlan.registerMutation("release.m117", {
      mode: "first",
      source: releaseWorkflowFixtureSource,
      needle: "      - name: Verify exact npm provenance without credentials",
      replacement: "      - name: Skipped npm provenance",
      expectedOccurrences: 1,
      witness: {
        kind: "token",
        anchor: "      - name: Verify exact npm provenance without credentials",
        before: 1,
        after: 0
      }
    });
    releaseMutationPlan.registerCase({
      id: "release.case.m117",
      root: releaseMutationM117,
      checks: [
        {
          invoke: {
            kind: "npm.workflow",
          ...
          expectation: {
            id: "release.expectation.m117.primary",
            kind: "problem",
            problem:
              "npm provenance must bind the tag-push context before the sole publish and verify two exact attestations without credentials"
          }
    ...
    const releaseMutationM118 = releaseMutationPlan.registerMutation("release.m118", {
      ...
      source: releaseWorkflowFixtureSource,
      // biome-ignore lint/suspicious/noTemplateCurlyInString: GitHub expression must remain a literal StringLiteral.
      needle: "PUBLISH_ATTEMPTED: ${{ steps.npm_publication.outputs.publish_attempted }}",
      replacement: 'PUBLISH_ATTEMPTED: "false"',
      expectedOccurrences: 1,
      witness: {
        kind: "token",
        // biome-ignore lint/suspicious/noTemplateCurlyInString: GitHub expression must remain a literal StringLiteral.
        anchor: "PUBLISH_ATTEMPTED: ${{ steps.npm_publication.outputs.publish_attempted }}",
        before: 1,
        after: 0
      }
    });
    releaseMutationPlan.registerCase({
      id: "release.case.m118",
      root: releaseMutationM118,
      checks: [
        {
          invoke: {
            kind: "npm.workflow",
          ...
          expectation: {
            id: "release.expectation.m118.primary",
            kind: "problem",
            problem:
              "npm provenance must bind the tag-push context before the sole publish and verify two exact attestations without credentials"
          }
    ...
    releaseMutationPlan.registerCase({
      id: "release.case.m119",
      root: releaseMutationM119,
      checks: [
        {
          invoke: {
            kind: "npm.workflow",
          ...
          expectation: {
            id: "release.expectation.m119.primary",
            kind: "problem",
            problem:
              "npm provenance must bind the tag-push context before the sole publish and verify two exact attestations without credentials"
          }
    ...
    releaseMutationPlan.registerCase({
      id: "release.case.m120",
      root: releaseMutationM120,
      checks: [
        {
          invoke: {
            kind: "npm.workflow",
          ...
          expectation: {
            id: "release.expectation.m120.primary",
            kind: "problem",
            problem:
              "npm provenance must bind the tag-push context before the sole publish and verify two exact attestations without credentials"
          }
    ...
    releaseMutationPlan.registerCase({
      id: "release.case.m121",
      root: releaseMutationM121,
      checks: [
        {
          invoke: {
            kind: "npm.workflow",
          ...
          expectation: {
            id: "release.expectation.m121.primary",
            kind: "problem",
            problem:
              "npm provenance must bind the tag-push context before the sole publish and verify two exact attestations without credentials"
          }
    ...
    releaseMutationPlan.registerCase({
      id: "release.case.m122",
      root: releaseMutationM122,
      checks: [
        {
          invoke: {
            kind: "npm.workflow",
          ...
          expectation: {
            id: "release.expectation.m122.primary",
            kind: "problem",
            problem:
              "npm provenance must bind the tag-push context before the sole publish and verify two exact attestations without credentials"
          }
    ...
    releaseMutationPlan.registerCase({
      id: "release.case.m123",
      root: releaseMutationM123,
      checks: [
        {
          invoke: {
            kind: "npm.workflow",
          ...
          expectation: {
            id: "release.expectation.m123.primary",
            kind: "problem",
            problem:
              "npm provenance must bind the tag-push context before the sole publish and verify two exact attestations without credentials"
          }
    ...
    releaseMutationPlan.registerCase({
      id: "release.case.m124",
      root: releaseMutationM124,
      checks: [
        {
          invoke: {
            kind: "npm.workflow",
          ...
          expectation: {
            id: "release.expectation.m124.primary",
            kind: "problem",
            problem:
              "npm provenance must bind the tag-push context before the sole publish and verify two exact attestations without credentials"
          }
    ...
    const releaseMutationM125 = releaseMutationPlan.registerMutation("release.m125", {
      mode: "first",
      source: releaseWorkflowFixtureSource,
      needle: '          NPM_GLOBALCONFIG="$VERIFY_ROOT/global.npmrc"',
      replacement: '          NPM_GLOBALCONFIG="$VERIFY_ROOT/user.npmrc"',
      expectedOccurrences: 1,
      witness: {
        kind: "token",
        anchor: '          NPM_GLOBALCONFIG="$VERIFY_ROOT/global.npmrc"',
        before: 1,
        after: 0
      }
    });
    releaseMutationPlan.registerCase({
      id: "release.case.m125",
      root: releaseMutationM125,
      checks: [
        {
          invoke: {
            kind: "npm.workflow",
          ...
          expectation: {
            id: "release.expectation.m125.primary",
            kind: "problem",
            problem:
              "npm provenance must bind the tag-push context before the sole publish and verify two exact attestations without credentials"
          }
    ...
    const releaseMutationM126 = releaseMutationPlan.registerMutation("release.m126", {
      mode: "first",
      source: releaseWorkflowFixtureSource,
      needle: '/usr/bin/touch "$NPM_USERCONFIG" "$NPM_GLOBALCONFIG"',
      replacement: '/bin/true "$NPM_USERCONFIG" "$NPM_GLOBALCONFIG"',
      expectedOccurrences: 1,
      witness: {
        kind: "token",
        anchor: '/usr/bin/touch "$NPM_USERCONFIG" "$NPM_GLOBALCONFIG"',
        before: 1,
        after: 0
      }
    });
    releaseMutationPlan.registerCase({
      id: "release.case.m126",
      root: releaseMutationM126,
      checks: [
        {
          invoke: {
            kind: "npm.workflow",
          ...
          expectation: {
            id: "release.expectation.m126.primary",
            kind: "problem",
            problem:
              "npm provenance must bind the tag-push context before the sole publish and verify two exact attestations without credentials"
          }
    ...
    const releaseMutationM127 = releaseMutationPlan.registerMutation("release.m127", {
      mode: "first",
      source: releaseWorkflowFixtureSource,
      needle: '/bin/chmod 600 "$NPM_USERCONFIG" "$NPM_GLOBALCONFIG"',
      replacement: '/bin/chmod 644 "$NPM_USERCONFIG" "$NPM_GLOBALCONFIG"',
      expectedOccurrences: 1,
      witness: {
        kind: "token",
        anchor: '/bin/chmod 600 "$NPM_USERCONFIG" "$NPM_GLOBALCONFIG"',
        before: 1,
        after: 0
      }
    });
    releaseMutationPlan.registerCase({
      id: "release.case.m127",
      root: releaseMutationM127,
      checks: [
        {
          invoke: {
            kind: "npm.workflow",
          ...
          expectation: {
            id: "release.expectation.m127.primary",
            kind: "problem",
            problem:
              "npm provenance must bind the tag-push context before the sole publish and verify two exact attestations without credentials"
          }
    ...
    const releaseMutationM128 = releaseMutationPlan.registerMutation("release.m128", {
      mode: "first",
      source: releaseWorkflowFixtureSource,
      needle: '"NPM_CONFIG_USERCONFIG=$NPM_USERCONFIG"',
      replacement: '"NPM_CONFIG_USERCONFIG=/dev/null"',
      expectedOccurrences: 1,
      witness: {
        kind: "token",
        anchor: '"NPM_CONFIG_USERCONFIG=$NPM_USERCONFIG"',
        before: 1,
        after: 0
      }
    });
    releaseMutationPlan.registerCase({
      id: "release.case.m128",
      root: releaseMutationM128,
      checks: [
        {
          invoke: {
            kind: "npm.workflow",
          ...
          expectation: {
            id: "release.expectation.m128.primary",
            kind: "problem",
            problem:
              "npm provenance must bind the tag-push context before the sole publish and verify two exact attestations without credentials"
          }
    ...
    const releaseMutationM129 = releaseMutationPlan.registerMutation("release.m129", {
      mode: "first",
      source: releaseWorkflowFixtureSource,
      needle: '"NPM_CONFIG_PREFER_ONLINE=true"',
      replacement: '"NPM_CONFIG_PREFER_ONLINE=false"',
      expectedOccurrences: 1,
      witness: {
        kind: "token",
        anchor: '"NPM_CONFIG_PREFER_ONLINE=true"',
        before: 1,
        after: 0
      }
    });
    releaseMutationPlan.registerCase({
      id: "release.case.m129",
      root: releaseMutationM129,
      checks: [
        {
          invoke: {
            kind: "npm.workflow",
          ...
          expectation: {
            id: "release.expectation.m129.primary",
            kind: "problem",
            problem:
              "npm provenance must bind the tag-push context before the sole publish and verify two exact attestations without credentials"
          }
    ...
    const releaseMutationM130 = releaseMutationPlan.registerMutation("release.m130", {
      mode: "first",
      source: releaseWorkflowFixtureSource,
      needle: "--max-filesize 4194304 --retry 0",
      replacement: "--max-filesize 4194304 --retry 1",
      expectedOccurrences: 1,
      witness: {
        kind: "token",
        anchor: "--max-filesize 4194304 --retry 0",
        before: 1,
        after: 0
      }
    });
    releaseMutationPlan.registerCase({
      id: "release.case.m130",
      root: releaseMutationM130,
      checks: [
        {
          invoke: {
            kind: "npm.workflow",
          ...
          expectation: {
            id: "release.expectation.m130.primary",
            kind: "problem",
            problem:
              "npm provenance must bind the tag-push context before the sole publish and verify two exact attestations without credentials"
          }
    ...
    const releaseMutationM131 = releaseMutationPlan.registerMutation("release.m131", {
      mode: "first",
      source: releaseWorkflowFixtureSource,
      needle: '[ "$NPM_CLI_ACTUAL_SRI" != "$NPM_CLI_SRI" ]',
      replacement: "false",
      expectedOccurrences: 1,
      witness: {
        kind: "token",
        anchor: '[ "$NPM_CLI_ACTUAL_SRI" != "$NPM_CLI_SRI" ]',
        before: 1,
        after: 0
      }
    });
    releaseMutationPlan.registerCase({
      id: "release.case.m131",
      root: releaseMutationM131,
      checks: [
        {
          invoke: {
            kind: "npm.workflow",
          ...
          expectation: {
            id: "release.expectation.m131.primary",
            kind: "problem",
            problem:
              "npm provenance must bind the tag-push context before the sole publish and verify two exact attestations without credentials"
          }
    ...
    releaseMutationPlan.registerCase({
      id: "release.case.m132",
      root: releaseMutationM132,
      checks: [
        {
          invoke: {
            kind: "npm.workflow",
          ...
          expectation: {
            id: "release.expectation.m132.primary",
            kind: "problem",
            problem:
              "npm provenance must bind the tag-push context before the sole publish and verify two exact attestations without credentials"
          }
    ...
    releaseMutationPlan.registerCase({
      id: "release.case.m133",
      root: releaseMutationM133,
      checks: [
        {
          invoke: {
            kind: "npm.workflow",
          ...
          expectation: {
            id: "release.expectation.m133.primary",
            kind: "problem",
            problem:
              "npm provenance must bind the tag-push context before the sole publish and verify two exact attestations without credentials"
          }
    ...
    releaseMutationPlan.registerCase({
      id: "release.case.m134",
      root: releaseMutationM134,
      checks: [
        {
          invoke: {
            kind: "npm.workflow",
          ...
          expectation: {
            id: "release.expectation.m134.primary",
            kind: "problem",
            problem:
              "npm provenance must bind the tag-push context before the sole publish and verify two exact attestations without credentials"
          }
    ...
    const releaseMutationM135 = releaseMutationPlan.registerMutation("release.m135", {
      mode: "first",
      source: releaseWorkflowFixtureSource,
      needle: 'NF == 0 || (substr($0, 1, 1) != "-" && substr($0, 1, 1) != "d")',
      replacement: "false",
      expectedOccurrences: 1,
      witness: {
        kind: "token",
        anchor: 'NF == 0 || (substr($0, 1, 1) != "-" && substr($0, 1, 1) != "d")',
        before: 1,
        after: 0
      }
    });
    releaseMutationPlan.registerCase({
      id: "release.case.m135",
      root: releaseMutationM135,
      checks: [
        {
          invoke: {
            kind: "npm.workflow",
          ...
          expectation: {
            id: "release.expectation.m135.primary",
            kind: "problem",
            problem:
              "npm provenance must bind the tag-push context before the sole publish and verify two exact attestations without credentials"
          }
    ...
    const releaseMutationM136 = releaseMutationPlan.registerMutation("release.m136", {
      mode: "first",
      source: releaseWorkflowFixtureSource,
      needle: "--save-exact --package-lock=true --ignore-scripts --no-audit --no-fund --omit=optional",
      replacement: "--save-exact --package-lock=true --no-audit --no-fund --omit=optional",
      expectedOccurrences: 1,
      witness: {
        kind: "token",
        anchor: "--save-exact --package-lock=true --ignore-scripts --no-audit --no-fund --omit=optional",
        before: 1,
        after: 0
      }
    });
    releaseMutationPlan.registerCase({
      id: "release.case.m136",
      root: releaseMutationM136,
      checks: [
        {
          invoke: {
            kind: "npm.workflow",
          ...
          expectation: {
            id: "release.expectation.m136.primary",
            kind: "problem",
            problem:
              "npm provenance must bind the tag-push context before the sole publish and verify two exact attestations without credentials"
          }
    ...
    const releaseMutationM137 = releaseMutationPlan.registerMutation("release.m137", {
      mode: "first",
      source: releaseWorkflowFixtureSource,
      needle: '.packages["node_modules/@oomkapwn/enquire-mcp"].integrity == $integrity',
      replacement: '.packages["node_modules/@oomkapwn/enquire-mcp"].version == $version',
      expectedOccurrences: 1,
      witness: {
        kind: "token",
        anchor: '.packages["node_modules/@oomkapwn/enquire-mcp"].integrity == $integrity',
        before: 1,
        after: 0
      }
    });
    releaseMutationPlan.registerCase({
      id: "release.case.m137",
      root: releaseMutationM137,
      checks: [
        {
          invoke: {
            kind: "npm.workflow",
          ...
          expectation: {
            id: "release.expectation.m137.primary",
            kind: "problem",
            problem:
              "npm provenance must bind the tag-push context before the sole publish and verify two exact attestations without credentials"
          }
    ...
    releaseMutationPlan.registerCase({
      id: "release.case.m138",
      root: releaseMutationM138,
      checks: [
        {
          invoke: {
            kind: "npm.workflow",
          ...
          expectation: {
            id: "release.expectation.m138.primary",
            kind: "problem",
            problem:
              "npm provenance must bind the tag-push context before the sole publish and verify two exact attestations without credentials"
          }
    ...
    const releaseMutationM141 = releaseMutationPlan.registerMutation("release.m141", {
      mode: "first",
      source: releaseWorkflowFixtureSource,
      needle: "--json --include-attestations --omit=optional",
      replacement: "--json --omit=optional",
      expectedOccurrences: 1,
      witness: {
        kind: "token",
        anchor: "--json --include-attestations --omit=optional",
        before: 1,
        after: 0
      }
    });
    releaseMutationPlan.registerCase({
      id: "release.case.m141",
      root: releaseMutationM141,
      checks: [
        {
          invoke: {
            kind: "npm.workflow",
          ...
          expectation: {
            id: "release.expectation.m141.primary",
            kind: "problem",
            problem:
              "npm provenance must bind the tag-push context before the sole publish and verify two exact attestations without credentials"
          }
    ...
    const releaseMutationM142 = releaseMutationPlan.registerMutation("release.m142", {
      mode: "first",
      source: releaseWorkflowFixtureSource,
      needle: "--fetch-retries=0 --fetch-timeout=60000 --prefer-online",
      replacement: "--fetch-retries=0 --fetch-timeout=60000",
      expectedOccurrences: 1,
      witness: {
        kind: "token",
        anchor: "--fetch-retries=0 --fetch-timeout=60000 --prefer-online",
        before: 1,
        after: 0
      }
    });
    releaseMutationPlan.registerCase({
      id: "release.case.m142",
      root: releaseMutationM142,
      checks: [
        {
          invoke: {
            kind: "npm.workflow",
          ...
          expectation: {
            id: "release.expectation.m142.primary",
            kind: "problem",
            problem:
              "npm provenance must bind the tag-push context before the sole publish and verify two exact attestations without credentials"
          }
    ...
    const releaseMutationM143 = releaseMutationPlan.registerMutation("release.m143", {
      mode: "first",
      source: releaseWorkflowFixtureSource,
      needle: `/usr/bin/env -i "\${CLEAN_ENV[@]}" \\\n              ${NPM_PROVENANCE_EVALUATOR_COMMAND}`,
      replacement: `              ${NPM_PROVENANCE_EVALUATOR_COMMAND}`,
      expectedOccurrences: 1,
      witness: {
        kind: "token",
        anchor: `/usr/bin/env -i "\${CLEAN_ENV[@]}" \\\n              ${NPM_PROVENANCE_EVALUATOR_COMMAND}`,
        before: 1,
        after: 0
      }
    });
    releaseMutationPlan.registerCase({
      id: "release.case.m143",
      root: releaseMutationM143,
      checks: [
        {
          invoke: {
            kind: "npm.workflow",
          ...
          expectation: {
            id: "release.expectation.m143.primary",
            kind: "problem",
            problem:
              "npm provenance must bind the tag-push context before the sole publish and verify two exact attestations without credentials"
          }
    ...
    const releaseMutationM145 = releaseMutationPlan.registerMutation("release.m145", {
      ...
      needle: " --kill-after=5s",
      replacement: "",
      expectedOccurrences: 1,
      witness: {
        kind: "token",
        anchor: " --kill-after=5s",
        before: 1,
        after: 0
      }
    });
    const releaseMutationM144 = releaseMutationPlan.registerMutation("release.m144", {
      mode: "first",
      source: releaseWorkflowFixtureSource,
      needle: NPM_PROVENANCE_EVALUATOR_COMMAND,
      replacement: releaseMutationM145,
      expectedOccurrences: 1,
      witness: {
        kind: "token",
        anchor: NPM_PROVENANCE_EVALUATOR_COMMAND,
        before: 1,
        after: 0
      }
    });
    releaseMutationPlan.registerCase({
      id: "release.case.m144",
      root: releaseMutationM144,
      checks: [
        {
          invoke: {
            kind: "npm.workflow",
          ...
          expectation: {
            id: "release.expectation.m144.primary",
            kind: "problem",
            problem:
              "npm provenance must bind the tag-push context before the sole publish and verify two exact attestations without credentials"
          }
    ...
    const releaseMutationM146 = releaseMutationPlan.registerMutation("release.m146", {
      mode: "first",
      source: releaseWorkflowFixtureSource,
      needle: "for (( attempt=1; attempt<=8; attempt++ )); do",
      replacement: "for (( attempt=1; attempt<=1; attempt++ )); do",
      expectedOccurrences: 1,
      witness: {
        kind: "token",
        anchor: "for (( attempt=1; attempt<=8; attempt++ )); do",
        before: 1,
        after: 0
      }
    });
    releaseMutationPlan.registerCase({
      id: "release.case.m146",
      root: releaseMutationM146,
      checks: [
        {
          invoke: {
            kind: "npm.workflow",
          ...
          expectation: {
            id: "release.expectation.m146.primary",
            kind: "problem",
            problem:
              "npm provenance must bind the tag-push context before the sole publish and verify two exact attestations without credentials"
          }
    ...
    const releaseMutationM148 = releaseMutationPlan.registerMutation("release.m148", {
      mode: "first",
      source: releaseWorkflowFixtureSource,
      needle: "attempt $attempt/8",
      replacement: "attempt $attempt/7",
      expectedOccurrences: 1,
      witness: {
        kind: "token",
        anchor: "attempt $attempt/8",
        before: 1,
        after: 0
      }
    });
    releaseMutationPlan.registerCase({
      id: "release.case.m148",
      root: releaseMutationM148,
      checks: [
        {
          invoke: {
            kind: "npm.workflow",
          ...
          expectation: {
            id: "release.expectation.m148.primary",
            kind: "problem",
            problem:
              "npm provenance must bind the tag-push context before the sole publish and verify two exact attestations without credentials"
          }
    ...
    releaseMutationPlan.registerCase({
      id: "release.case.m149",
      root: releaseMutationM149,
      checks: [
        {
          invoke: {
            kind: "npm.workflow",
          ...
          expectation: {
            id: "release.expectation.m149.primary",
            kind: "problem",
            problem:
              "npm provenance must bind the tag-push context before the sole publish and verify two exact attestations without credentials"
          }
    ...
    const releaseMutationM150 = releaseMutationPlan.registerMutation("release.m150", {
      mode: "first",
      source: releaseWorkflowFixtureSource,
      needle: NPM_PROVENANCE_SUCCESS_CONDITION,
      replacement: '[ "$AUDIT_EXIT" -eq 0 ] || [ "$EVALUATOR_EXIT" -eq 0 ]',
      expectedOccurrences: 1,
      witness: {
        kind: "token",
        anchor: NPM_PROVENANCE_SUCCESS_CONDITION,
        before: 1,
        after: 0
      }
    });
    releaseMutationPlan.registerCase({
      id: "release.case.m150",
      root: releaseMutationM150,
      checks: [
        {
          invoke: {
            kind: "npm.workflow",
          ...
          expectation: {
            id: "release.expectation.m150.primary",
            kind: "problem",
            problem:
              "npm provenance must bind the tag-push context before the sole publish and verify two exact attestations without credentials"
          }
    ...
    const releaseMutationM153 = releaseMutationPlan.registerMutation("release.m153", {
      mode: "first",
      source: releaseIntegritySource,
      needle: "statement.subject.length !== 1",
      replacement: "statement.subject.length < 1",
      expectedOccurrences: 1,
      witness: {
        kind: "token",
        anchor: "statement.subject.length !== 1",
        before: 1,
        after: 0
      }
    });
    releaseMutationPlan.registerCase({
      id: "release.case.m153",
      root: releaseMutationM153,
      checks: [
        {
          invoke: {
            kind: "npm.evaluator",
          ...
          expectation: {
            id: "release.expectation.m153.primary",
            kind: "problem",
            problem:
              "npm provenance must bind the tag-push context before the sole publish and verify two exact attestations without credentials"
          }
    ...
    const releaseMutationM154 = releaseMutationPlan.registerMutation("release.m154", {
      mode: "first",
      source: releaseIntegritySource,
      needle: "subject.name !== expectedPurl || digest.sha512 !== expectedSha512",
      replacement: "subject.name !== expectedPurl && digest.sha512 !== expectedSha512",
      expectedOccurrences: 1,
      witness: {
        kind: "token",
        anchor: "subject.name !== expectedPurl || digest.sha512 !== expectedSha512",
        before: 1,
        after: 0
      }
    });
    releaseMutationPlan.registerCase({
      id: "release.case.m154",
      root: releaseMutationM154,
      checks: [
        {
          invoke: {
            kind: "npm.evaluator",
          ...
          expectation: {
            id: "release.expectation.m154.primary",
            kind: "problem",
            problem:
              "npm provenance must bind the tag-push context before the sole publish and verify two exact attestations without credentials"
          }
    ...
    const releaseMutationM155 = releaseMutationPlan.registerMutation("release.m155", {
      mode: "first",
      source: releaseIntegritySource,
      needle: "predicateType === NPM_PROVENANCE_IDENTITY.publishPredicateType",
      replacement: "predicateType === NPM_PROVENANCE_IDENTITY.slsaPredicateType",
      expectedOccurrences: 1,
      witness: {
        kind: "token",
        anchor: "predicateType === NPM_PROVENANCE_IDENTITY.publishPredicateType",
        before: 1,
        after: 0
      }
    });
    releaseMutationPlan.registerCase({
      id: "release.case.m155",
      root: releaseMutationM155,
      checks: [
        {
          invoke: {
            kind: "npm.evaluator",
          ...
          expectation: {
            id: "release.expectation.m155.primary",
            kind: "problem",
            problem:
              "npm provenance must bind the tag-push context before the sole publish and verify two exact attestations without credentials"
          }
    ...
    const releaseMutationM156 = releaseMutationPlan.registerMutation("release.m156", {
      mode: "first",
      source: releaseIntegritySource,
      needle: '["publicKey", "tlogEntries", "timestampVerificationData"]',
      replacement: '["x509CertificateChain", "tlogEntries", "timestampVerificationData"]',
      expectedOccurrences: 1,
      witness: {
        kind: "token",
        anchor: '["publicKey", "tlogEntries", "timestampVerificationData"]',
        before: 1,
        after: 0
      }
    });
    releaseMutationPlan.registerCase({
      id: "release.case.m156",
      root: releaseMutationM156,
      checks: [
        {
          invoke: {
            kind: "npm.evaluator",
          ...
          expectation: {
            id: "release.expectation.m156.primary",
            kind: "problem",
            problem:
              "npm provenance must bind the tag-push context before the sole publish and verify two exact attestations without credentials"
          }
    ...
    const releaseMutationM157 = releaseMutationPlan.registerMutation("release.m157", {
      mode: "first",
      source: releaseIntegritySource,
      needle: "verified.tlogEntries.length === 0",
      replacement: "false",
      expectedOccurrences: 1,
      witness: {
        kind: "token",
        anchor: "verified.tlogEntries.length === 0",
        before: 1,
        after: 0
      }
    });
    releaseMutationPlan.registerCase({
      id: "release.case.m157",
      root: releaseMutationM157,
      checks: [
        {
          invoke: {
            kind: "npm.evaluator",
          ...
          expectation: {
            id: "release.expectation.m157.primary",
            kind: "problem",
            problem:
              "npm provenance must bind the tag-push context before the sole publish and verify two exact attestations without credentials"
          }
    ...
    const releaseMutationM158 = releaseMutationPlan.registerMutation("release.m158", {
      mode: "first",
      source: releaseIntegritySource,
      needle: "!isRecord(verified.timestampVerificationData)",
      replacement: "false",
      expectedOccurrences: 1,
      witness: {
        kind: "token",
        anchor: "!isRecord(verified.timestampVerificationData)",
        before: 1,
        after: 0
      }
    });
    releaseMutationPlan.registerCase({
      id: "release.case.m158",
      root: releaseMutationM158,
      checks: [
        {
          invoke: {
            kind: "npm.evaluator",
          ...
          expectation: {
            id: "release.expectation.m158.primary",
            kind: "problem",
            problem:
              "npm provenance must bind the tag-push context before the sole publish and verify two exact attestations without credentials"
          }
    ...
    const releaseMutationM159 = releaseMutationPlan.registerMutation("release.m159", {
      mode: "first",
      source: releaseIntegritySource,
      needle: "/^SHA256:[A-Za-z0-9+/]{43}$/u.test(publicKey.hint)",
      replacement: 'publicKey.hint.startsWith("SHA256:")',
      expectedOccurrences: 1,
      witness: {
        kind: "token",
        anchor: "/^SHA256:[A-Za-z0-9+/]{43}$/u.test(publicKey.hint)",
        before: 1,
        after: 0
      }
    });
    releaseMutationPlan.registerCase({
      id: "release.case.m159",
      root: releaseMutationM159,
      checks: [
        {
          invoke: {
            kind: "npm.evaluator",
          ...
          expectation: {
            id: "release.expectation.m159.primary",
            kind: "problem",
            problem:
              "npm provenance must bind the tag-push context before the sole publish and verify two exact attestations without credentials"
          }
    ...
    const releaseMutationM160 = releaseMutationPlan.registerMutation("release.m160", {
      mode: "first",
      source: releaseIntegritySource,
      needle: "keyid !== publicKey.hint",
      replacement: "false",
      expectedOccurrences: 1,
      witness: {
        kind: "token",
        anchor: "keyid !== publicKey.hint",
        before: 1,
        after: 0
      }
    });
    releaseMutationPlan.registerCase({
      id: "release.case.m160",
      root: releaseMutationM160,
      checks: [
        {
          invoke: {
            kind: "npm.evaluator",
          ...
          expectation: {
            id: "release.expectation.m160.primary",
            kind: "problem",
            problem:
              "npm provenance must bind the tag-push context before the sole publish and verify two exact attestations without credentials"
          }
    ...
    const releaseMutationM161 = releaseMutationPlan.registerMutation("release.m161", {
      mode: "first",
      source: releaseIntegritySource,
      needle: "chain.certificates.length !== 1",
      replacement: "chain.certificates.length < 1",
      expectedOccurrences: 1,
      witness: {
        kind: "token",
        anchor: "chain.certificates.length !== 1",
        before: 1,
        after: 0
      }
    });
    releaseMutationPlan.registerCase({
      id: "release.case.m161",
      root: releaseMutationM161,
      checks: [
        {
          invoke: {
            kind: "npm.evaluator",
          ...
          expectation: {
            id: "release.expectation.m161.primary",
            kind: "problem",
            problem:
              "npm provenance must bind the tag-push context before the sole publish and verify two exact attestations without credentials"
          }
    ...
    const releaseMutationM162 = releaseMutationPlan.registerMutation("release.m162", {
      mode: "first",
      source: releaseIntegritySource,
      needle: "decodeCanonicalBase64(certificate.rawBytes",
      replacement: "Buffer.from(certificate.rawBytes",
      expectedOccurrences: 1,
      witness: {
        kind: "token",
        anchor: "decodeCanonicalBase64(certificate.rawBytes",
        before: 1,
        after: 0
      }
    });
    releaseMutationPlan.registerCase({
      id: "release.case.m162",
      root: releaseMutationM162,
      checks: [
        {
          invoke: {
            kind: "npm.evaluator",
          ...
          expectation: {
            id: "release.expectation.m162.primary",
            kind: "problem",
            problem:
              "npm provenance must bind the tag-push context before the sole publish and verify two exact attestations without credentials"
          }
    ...
    const releaseMutationM163 = releaseMutationPlan.registerMutation("release.m163", {
      mode: "first",
      source: releaseIntegritySource,
      needle: 'import { X509Certificate } from "node:crypto";',
      replacement: "const X509Certificate = undefined;",
      expectedOccurrences: 1,
      witness: {
        kind: "token",
        anchor: 'import { X509Certificate } from "node:crypto";',
        before: 1,
        after: 0
      }
    });
    releaseMutationPlan.registerCase({
      id: "release.case.m163",
      root: releaseMutationM163,
      checks: [
        {
          invoke: {
            kind: "npm.evaluator",
          ...
          expectation: {
            id: "release.expectation.m163.primary",
            kind: "problem",
            problem:
              "npm provenance must bind the tag-push context before the sole publish and verify two exact attestations without credentials"
          }
