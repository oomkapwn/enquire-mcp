...
The index file lives at `~/Library/Caches/enquire/<vault-hash>.fts5.db` (macOS) or `~/.cache/enquire/<vault-hash>.fts5.db` (Linux) by default. Override with `--index-file <path>`. Enquire best-effort reasserts `0600` on DB + WAL + SHM files after an admitted open where POSIX modes work; a chmod failure does not abort the completed open. A parent created by Enquire starts at `0700`, while an existing/custom parent remains operator-managed. See [SECURITY.md "Persistent FTS5 index: privacy posture"](../SECURITY.md#persistent-fts5-index-privacy-posture) for full privacy details.
